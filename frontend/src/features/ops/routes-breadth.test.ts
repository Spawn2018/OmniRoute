...
describe("U-routes-breadth standing", () => {
  it("records Charge 1.0–1.2 routes and refuses a 70-module claim", () => {
    ...
    expect(SHIPPED_CHARGE_ROUTES["625.0"]).toBe("/groupage-tariffs")
    expect(SHIPPED_CHARGE_ROUTES["626.0"]).toBe("/network-print-requirements")
    expect(SHIPPED_CHARGE_ROUTES["627.0"]).toBe("/document-templates")
    expect(SHIPPED_CHARGE_ROUTES["628.0"]).toBe("/shipments")
    expect(SHIPPED_CHARGE_ROUTES["629.0"]).toBe("/pallet-ledgers")
    ...
    expect(SHIPPED_CHARGE_ROUTES["633.0"]).toBe("/local-charges")
    ...
    expect(SHIPPED_CHARGE_ROUTES["642.0"]).toBe("/dangerous-goods")
    expect(SHIPPED_CHARGE_ROUTES["643.0"]).toBe("/dangerous-goods")
    expect(SHIPPED_CHARGE_ROUTES["644.0"]).toBe("/dangerous-goods")
    expect(SHIPPED_CHARGE_ROUTES["645.0"]).toBe("/claims")
    expect(SHIPPED_CHARGE_ROUTES["646.0"]).toBe("/rate-lines")
    expect(SHIPPED_CHARGE_ROUTES["647.0"]).toBe("/rate-lines")
    expect(SHIPPED_CHARGE_ROUTES["648.0"]).toBe("/rate-lines")
    expect(SHIPPED_CHARGE_ROUTES["649.0"]).toBe("/charge-codes")
    expect(SHIPPED_CHARGE_ROUTES["650.0"]).toBe("/charge-codes")
    expect(SHIPPED_CHARGE_ROUTES["651.0"]).toBe("/rate-lines")
    expect(SHIPPED_CHARGE_ROUTES["652.0"]).toBe("/quotations")
    expect(SHIPPED_CHARGE_ROUTES["653.0"]).toBe("/quotations")
    expect(SHIPPED_CHARGE_ROUTES["654.0"]).toBe("/quotations")
    expect(SHIPPED_CHARGE_ROUTES["655.0"]).toBe("/quotations")
    expect(SHIPPED_CHARGE_ROUTES["636.0"]).toBe("/document-templates")
    expect(Object.keys(SHIPPED_CHARGE_ROUTES)).toHaveLength(595)

    expect(OPS_JOBS.map((job) => job.route)).toEqual(
      expect.arrayContaining(Object.values(SHIPPED_CHARGE_ROUTES)),
    )
    expect(OPS_JOBS).toHaveLength(349)
