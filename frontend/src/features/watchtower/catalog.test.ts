...
describe("watchtower surface for 94.0", () => {
  ...
  it("shows board counts from already fetched lists without a map library", () => {
    const page = readSource("features/watchtower/catalog-page.tsx")
    const ops = readSource("features/ops/ops-index.ts")
    expect(ops).toContain('"124.0": "/watchtower"')
    expect(ops).toContain("Wieża — liczniki")
    expect(page).toContain('data-watchtower="tiles"')
    expect(page).toContain("data-watchtower-tile={args.kind}")
    expect(page).toContain('kind="exceptions"')
    expect(page).toContain('kind="pending"')
    expect(page).toContain('kind="drafts"')
    expect(page).toContain("pendingRows")
    expect(page).not.toContain("createMailDraft")
    expect(page).not.toContain("leaflet")
    expect(page).not.toContain("mapbox")
    expect(page).not.toContain("ais")
