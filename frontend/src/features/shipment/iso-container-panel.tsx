...
export function IsoContainerPanel(args: { signedIn: boolean }) {
  ...
  const persist = useMutation({
    mutationFn: () =>
      saveContainer({
        ...
        volume_m3: optionalToken(cube),
        pickup_date: optionalToken(pickup),
        return_date: optionalToken(returned),
        gate_in_date: optionalToken(gateIn),
        delivery_date: optionalToken(delivered),
  ...
  return (
    <section className="grid gap-2 rounded-md border border-border p-3" data-container="iso">
      ...
      <label className="flex flex-col gap-1 text-xs">
        Objętość HITL (opcjonalnie)
        <Input
          aria-label="Objętość kontenera"
          placeholder="volume_m3"
          value={cube}
          onChange={(event) => setCube(event.target.value)}
        />
      </label>
      <label className="flex flex-col gap-1 text-xs">
        Data odbioru HITL (opcjonalnie)
        <Input
          aria-label="Data odbioru kontenera"
      ...
      <label className="flex flex-col gap-1 text-xs">
        Data zwrotu HITL (opcjonalnie)
        <Input
          aria-label="Data zwrotu kontenera"
      ...
      <label className="flex flex-col gap-1 text-xs">
        Data wjazdu HITL (opcjonalnie)
        <Input
          aria-label="Data wjazdu kontenera"
          placeholder="gate_in_date"
          value={gateIn}
          onChange={(event) => setGateIn(event.target.value)}
        />
      </label>
      <label className="flex flex-col gap-1 text-xs">
        Data dostawy HITL (opcjonalnie)
        <Input
          aria-label="Data dostawy kontenera"
      ...
      <label className="flex flex-col gap-1 text-xs">
        Data rozładunku HITL (opcjonalnie)
        <Input
          aria-label="Data rozładunku kontenera"
          placeholder="unload_date"
          value={unloaded}
          onChange={(event) => setUnloaded(event.target.value)}
        />
      </label>
      <label className="flex flex-col gap-1 text-xs">
        Temp. min HITL (opcjonalnie)
        <Input
          aria-label="Temperatura minimalna kontenera"
      ...
      <ul className="space-y-1">
        {(listed.data ?? []).map((row) => (
          <li key={row.id} className="font-mono text-xs">
            ...
            {row.volume_m3 !== null ? ` · ${row.volume_m3} m3` : ""}
            {row.pickup_date !== null ? ` · odbiór ${row.pickup_date}` : ""}
            {row.return_date !== null ? ` · zwrot ${row.return_date}` : ""}
            {row.gate_in_date !== null ? ` · wjazd ${row.gate_in_date}` : ""}
            {row.delivery_date !== null ? ` · dostawa ${row.delivery_date}` : ""}
