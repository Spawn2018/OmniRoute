...
| wycena | quotation | opcjonalny `valid_until` DATE HITL (652.0); opcjonalny `revision_no` INTEGER HITL ≥ 1 (653.0); opcjonalny `mqc_teu` Decimal HITL (654.0); opcjonalny `mqc_window` tekst HITL 1–64 (655.0); nie Deadline Engine; nie float; nie MQC SQL |
...
| punkt operacyjny | stop | T1; ZA/WY na zleceniu; miejsce ze słownika; 213.0 opcjonalny `stop_group_code`; 215.0 opcjonalny `notes_for_driver`; 247.0 opcjonalny `weight_kg`; 248.0 opcjonalny `quantity`; 249.0 opcjonalny `packaging_code`; 250.0 opcjonalny `seal_in`; 251.0 opcjonalny `seal_out`; 252.0 opcjonalny `appointment_ref`; 603.0 opcjonalny `appointment_status` (`noted`/`advised`/`confirmed`/`cancelled`); 604.0 opcjonalny `no_show_at`; 253.0 opcjonalny `waiting_free_minutes`; 254.0 opcjonalny `waiting_started_at`; 255.0 opcjonalny `pod_quality`; nie mapa |
| grupa punktów | stop_group | T1b; HITL nagłówek grupy na zleceniu (`group_code` + `shipment_id`); 213.0 `stop_group_code` na punkcie zostaje; 601.0 opcjonalny `stop.stop_group_id`; 602.0 fill `stop_group_code` z FK; nie mapa |
| zasób floty | resource | T2; pojazd / kierowca / naczepa; 259.0 trasa `/fleet`; 589.0 `capacity_kg` / 590.0 `capacity_ldm` / 591.0 `capacity_m3` HITL; 592.0 `resource_document` (rodzaj + data, nie odliczanie); 593.0 `inventory_no` HITL; 594.0 `adr_certified` HITL; 595.0 `reefer` HITL; 596.0 `tail_lift` HITL; 597.0 `phone` HITL; 598.0 `driver_card_no` HITL; 599.0 `vehicle_profile` HITL; nie trip; nie GPS |
...
| kontener | container | T3; ISO 6346 + iso_size_type; 216.0–218.0 trzy plomby; 219.0 `vessel_name`; 220.0 `voyage_no`; 221.0 `remarks`; 222.0 `cargo_description`; 223.0 `packaging_code`; 607.0 `grade` (HITL tekst, nie IICL live); 608.0 `vessel_imo` (HITL tekst, nie AIS); 609.0 `alliance_service` (HITL tekst, nie live alliance); 610.0 `pol_unlocode` (HITL UN/LOCODE, nie FK portu); 611.0 `pod_unlocode` (HITL UN/LOCODE, nie FK portu); 612.0 `destination_city` (HITL tekst, nie geokoder); 224.0 `ref_1`; 225.0 `ref_2`; 226.0 `ref_3`; 227.0 `ref_4`; 228.0 `ref_5`; 229.0 `reefer`; 230.0 `pickup_terminal`; 231.0 `return_terminal`; 232.0 `bl_kind` (nie HBL); 233.0 `free_time_origin_h` (nie odliczanie); 234.0 `free_time_dest_h` (nie odliczanie); 566.0 `demurrage_free_days` (dni HITL, nie countdown); 567.0 `detention_free_days` (dni HITL, nie countdown); 568.0 `mixed_dd_days` (dni HITL, nie countdown); 235.0 `si_cutoff_at` (nie live HTTP); 236.0 `ams_cutoff_at` (nie live HTTP); 237.0 `cy_cutoff_at` (nie live HTTP); 238.0 `cfs_cutoff_at` (nie live HTTP); 239.0 `vgm_kg` / `vgm_method` / `vgm_cutoff_at` (nie kalkulator, nie live HTTP); 569.0 `tare_kg` (tara HITL Decimal, nie kalkulator, nie VGM); 570.0 `pin_code` (PIN odbioru HITL tekst, nie live terminal, nie ciphertext); 571.0 `payload_kg` (ładowność HITL Decimal, nie kalkulator, nie quantity); 572.0 `teu` (TEU HITL Decimal, nie kalkulator z typu ISO); 573.0 `quantity` (ilość HITL Integer, nie stop.quantity); 574.0 `weight_kg` (waga HITL Decimal, nie stop.weight_kg, nie VGM); 575.0 `volume_m3` (objętość HITL Decimal, nie kalkulator CBM, nie waga); 576.0 `pickup_date` (data odbioru HITL date, nie cutoff, nie countdown); 577.0 `return_date` (data zwrotu HITL date, nie cutoff, nie countdown); 578.0 `gate_in_date` (data wjazdu HITL date, nie cutoff, nie countdown); 579.0 `delivery_date` (data dostawy HITL date, nie cutoff, nie countdown); 580.0 `unload_date` (data rozładunku HITL date, nie cutoff, nie countdown); 581.0 `temp_min` (temp. min HITL Decimal °C, nie float); 582.0 `temp_max` (temp. max HITL Decimal °C, nie float); 583.0 `needs_external_power`; 584.0 `container_release_party_id` (FK party HITL zwolnienie, nie live); 585.0 egzekucja `temp_min` ≤ `temp_max` (gdy oba podane) (zasilanie HITL bool, nie N3); 240.0 `last_survey_at` (nie live HTTP); 241.0 `booking_no` (nie S21, nie live HTTP); 242.0 `carrier_party_id` (FK party, nie PIN, nie live HTTP); 243.0 `shipment_leg_id` (FK odcinka, nie PIN, nie live HTTP); nie temperatura; nie FK terminalu; nie PIN |
...
| dopasowanie podłogi | margin_match_mark | N6 HITL 639.0 DONE stance (`match`|`hold`|`waive`|`other`); nie matching SQL lane; nie auto charge |
