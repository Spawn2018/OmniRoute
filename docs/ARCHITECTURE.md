...
<!-- os-status:start -->
**Status:** **312.0** `/refaktor` CatalogSourceRefField (local/charge-template/cod). **Etap:** Plan (delta zaakceptowana `/noc`) — wolno `/plaster` **313.0**. **Następny:** **313.0** CI1 HITL `sla_clause` (delta `docs/deltas/open/313.0-sla-clause.md`). Plan: [PLAN-REALIZACJA.md](PLAN-REALIZACJA.md).
<!-- os-status:end --> 
