...
<!-- os-status:start -->
**Status:** **18.0** M-25 `offer_negotiation` przy `quotation` (zarchiwizowany). **Etap:** Plan. **Następny:** 19.0 M-26 `offer_document` przy `quotation` (`/plaster`). Podgląd faktów. Nie PDF. Nie U-print. Plan: [PLAN-REALIZACJA.md](PLAN-REALIZACJA.md).
<!-- os-status:end --> 
