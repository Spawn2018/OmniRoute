...
<!-- os-status:start -->
**Status:** **77.0** S14 `lock_version` na `operator_decision` (M-71). **Etap:** Plan. **Następny:** `/plaster` **78.0** — ingest Graph na `inbound_message` (M-32). `graph://` + `external_id`. Nie live HTTP. Nie send. Nie F9.1. Plan: [PLAN-REALIZACJA.md](PLAN-REALIZACJA.md).
<!-- os-status:end --> 
