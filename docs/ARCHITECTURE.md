...
<!-- os-status:start -->
**Status:** **329.0** G8 HITL `legal_hold_mark`. **Etap:** Plan (delta zaakceptowana `/noc`) — wolno `/plaster` **330.0**. **Następny:** **330.0** G10 HITL `company_mark` (delta `docs/deltas/open/330.0-company-mark.md`). Plan: [PLAN-REALIZACJA.md](PLAN-REALIZACJA.md).
<!-- os-status:end --> 
