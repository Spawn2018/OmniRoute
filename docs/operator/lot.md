...
Na `/air` zostaje lista portów z `airport` w `function_flags`. Osobno zapisujesz odcinek na zleceniu — noga z opcjonalnym numerem HAWB/MAWB albo nadaniem z puli prefiksu.
...
6. Odcinki `road`, `rail`, `china_rail` i `ocean_lcl` zostają osobnymi wierszami. System nie woła IATA i nie liczy cyfry kontrolnej.

Czego tu nie ma: cyfra kontrolna IATA, HTTP, e-rates, mapa, PDF. Marża zostaje na `/charges`. Konosament morski zostaje na `/ocean-bills`.

Nazwy w kodzie: `shipment_leg` · `leg_kind` · `air` · `hawb_no` · `mawb_no` · `hawb_number_prefix` · `mawb_number_prefix` · `airport` · `source_ref`.
