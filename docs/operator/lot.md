...
3. Wejdź na Lotniczy. Wklej `shipment_id`, start, koniec i `source_ref` (`fixture://shipment-leg/…` albo `tenant:manual`). HAWB i MAWB są opcjonalne (2–32 litery, cyfry, myślnik); MAWB w kształcie IATA (3 cyfry, opcjonalny myślnik, 8 cyfr) wymaga ostatniej cyfry równej serii 7 cyfr modulo 7, a zła cyfra odrzuca zapis — token spoza tego kształtu i HAWB zostają bez tej bramki.
...
7. Odcinki `road`, `rail`, `china_rail` i `ocean_lcl` zostają osobnymi wierszami. System nie woła IATA. Nadanie z puli nie dopisuje cyfry kontrolnej.

Czego tu nie ma: HTTP IATA, mapa, PDF. Marża zostaje na `/charges`. Konosament morski zostaje na `/ocean-bills`.
