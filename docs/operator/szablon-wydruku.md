...
3. „Zapisz szablon wydruku” z `source_ref` (`fixture://document-template/…` albo `tenant:manual`).

Czego tu nie ma: PDF/ZPL, QR `shipment_ref`, 409 wyjazdu, etykieta Palletforce, marża.

Nazwy w kodzie: `document_template` · `template_kind` · `layout_ref` · `output_kind` · `source_ref`.
