...
Nazwy w kodzie: `shipment` · `quotation_id` · `party_id` z wyceny · `shipment_ref` · `parent_shipment_id` · `relation_kind` · `trip` · `driver2_id` · `route_label` · `stop` · `stop_group_code` · `notes_for_driver` · `weight_kg` · `quantity`.
