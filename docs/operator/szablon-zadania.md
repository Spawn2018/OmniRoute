...
Czego tu nie ma: instancja `task` na `shipment`/`trip`/`stop`, matching SQL, worker outboxa, live KSeF, live VIES. Marża zostaje na `/charges`. Szablon opłat zostaje na `/charge-templates`.
