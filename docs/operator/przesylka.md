...
4. Opcjonalnie `stop_id` — punkt z tej samej trasy zlecenia. Obcy punkt odpada.
5. „Zapisz przesyłkę” wstawia wiersz.

Czego tu nie ma: paczka `/shipment-packages`, mapa, unique w bazie na FTL, kolumna trybu na zleceniu, SQL marży, live HTTP.

Nazwy w kodzie: `consignment` · `consignment_ref` · `shipment_id` · `stop_id` · `source_ref` · `load_kind` · `/consignments`.
