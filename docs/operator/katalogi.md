...
- zlecenie `/shipments` — zapis `shipment` z wyceny, strony (`shipment_stakeholder`; `party_id` wymagane) i instrukcji (`booking_instruction`; status dane, nie S21) oraz punktu (`stop`; miejsce ze słownika, strefa IANA, nie mapa) i zasobu (`resource`; pojazd/kierowca/naczepa, nie trip). Na `/quotations`: przeniesienie Incoterms (`field_carry_forward`; zmiana = nowy wiersz), checklista (`document_checklist_rule`; `blocks_dispatch` = dana, nie C8), macierz (`incoterm_responsibility`; seed Omni, nie cytat ICC) i adresat (`document_dispatch_rule`; rola, nie send). How-to: [zlecenie.md](zlecenie.md). Nie tracking. Nie kwota.
