...
- sieci `/networks` — katalog sieci oraz ręczny członek (`member_code` + nazwa + `party_id` kontrahenta) w wybranej sieci. Stary członek może nie mieć kontrahenta. „Zapisz zapytanie” dopisuje `carrier_inquiry` (`draft`). „Zapisz paczkę” — 1/wielu/wszyscy, status `queued`, opcjonalna trasa. Kwota tylko przy odpowiedzi. Top N z rankingu `answered` + „Zapisz szkice” (`mail_draft` / `carrier_inquiry`). Filtr kraju ISO na członkach (`party.country_code`). Nie portal. Nie live HTTP.
