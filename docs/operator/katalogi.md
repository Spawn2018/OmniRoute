...
- zlecenie `/shipments` — zapis `shipment` z wyceny. Na `/quotations`: przeniesienie Incoterms (`field_carry_forward`; zmiana = nowy wiersz), checklista (`document_checklist_rule`; `blocks_dispatch` = dana, nie C8) i macierz (`incoterm_responsibility`; seed Omni, nie cytat ICC). How-to: [zlecenie.md](zlecenie.md). Nie tracking. Nie kwota.
