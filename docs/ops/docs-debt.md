...
- **180.0 leftover:** bid/no-bid / four-eyes / TED / CO₂ (G2.11–G2.18). Tu tylko `tender_prospect` HITL + `source_ref`. Nie scrape. Nie kwota. Nie auto-award.
- **179.0 leftover:** prospecting HITL DONE 180.0. Zostaje bid/no-bid / four-eyes / TED / CO₂ (G2.11–G2.18). Tu tylko `draft_kind=tender_rfp` + accept → `tender_rfp_intake`. Nie zapis z LLM. Nie kwota. Nie auto-award.
