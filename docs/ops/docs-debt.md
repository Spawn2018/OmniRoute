...
- **272.0 leftover:** 273.0 Kod = opaque `blob_ciphertext` present/absent (nie szyfr). Zostaje `wrapped_dek` / `tenant_contract_kek` / KMS / unwrap; super-admin = bytea; CI1 `sla_clause`; CT7 p44/FourKites/Shippeo live (TO_VERIFY umowa). 272.0 = nagłówek. Nie KMS.
