...
- **187.0 leftover:** C1 `shipment_monitoring_filing` / SENT XML / PUESC HTTP / SENT-GEO; kolumny API/GEO/urząd na schemacie; seed mapy UE; C8 `party_document` + 409 na `POST shipment`; F1 live FA(3); G2.20–G2.22 `circle_sim`. Tu tylko `monitoring_scheme` HITL `scheme_code` + `source_ref`. Nie zgłoszenie. Nie kwota.
