...
- **265.0 leftover:** kółka / `circle_sim` / km ładowny (G2.20–G2.22); TT z actuals `stop`; what-if V8; 8 silników fizyki. Tu tylko `plan_snapshot` HITL trójka UUID bez FK + autor + czas + `source_ref`. Nie silnik. Nie mapa.
