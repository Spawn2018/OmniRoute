...
**650.0 PLAN:** seed Omni czterech kodów EXP1 na `charge_code` (`WAITING`/`NO_SHOW`/`DIVERSION`/`STAMP`) z `source_ref=omni:charge-code:exp1`. Nie CHECK. Nie matching. Nie float.
