...
- **7.0 leftover — klon mapped_column (dlaczego nie w tym plasterze):** `jscpd` 6 linii `charge_code.py` przeciw `dangerous_good.py`. Miksin = `refaktor-pass`. `just dup` 2,86% po kompaktowym modelu i osobnym formularzu UN/klasa (nie `CatalogCreateForm`).
