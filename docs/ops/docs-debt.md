...
- **12.0 leftover — ewaluacja warunku / zapis do charge (dlaczego nie w tym plasterze):** dopasowanie extra do wyceny wymaga silnika warunków i INSERT `charge` — teatr bez M-21 warunków portowych. Live taryfa terminalu = M-19. Parser `applies_when` = leftover. Klon nagłówka modelu vs `charge.py` — `refaktor-pass`. `just dup` 2,73%.
