...
- **153.0 leftover:** `seal_no_1` DONE 216.0. `seal_no_2` DONE 217.0. `seal_no_3` DONE 218.0. `vessel_name` DONE 219.0. `voyage_no` DONE 220.0. `remarks` DONE 221.0. `cargo_description` DONE 222.0. `packaging_code` DONE 223.0. `ref_1` DONE 224.0. `ref_2` DONE 225.0. Zostaje VGM/TARA Decimal; free_time; cutoffy; `bl_kind`; PIN; `ref_3`; reefer; party/terminal; TEU; `shipment_leg_id`; `booking_no`. Tu tylko ISO 6346 + typ + zlecenie + trzy plomby + statek + rejs + uwaga + ładunek + opakowanie + Ref 1–2. Nie booking.
...
- **225.0 leftover:** leftover T3 `ref_3`; PIN; VGM/cutoffy; booking. Tu tylko `ref_2` HITL. Nie waga. Nie PIN.
