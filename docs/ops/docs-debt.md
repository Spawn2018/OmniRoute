...
- **OS-1 (fabryka, poza osią Q/S, open):** uczenie i bezpieczne automatyzacje — [OS-1-fabryka-uczenie-i-automacje.md](../deltas/open/OS-1-fabryka-uczenie-i-automacje.md). Okno: piątkowa retro. Nie startuje zamiast S4. E1 (schemat bez MCP) osobnym commitem z `agentlint --write`. E2/E3 = propozycje bez zgody na implementację. Playbook: [multitask-playbook.md](multitask-playbook.md). Leftover z delty: lokalny `just gate` fail-fast (E2); pre-commit baseline agentlinta (E3); `.cursorignore` nie pokrywa `docs/_source/` z `excludePatterns` (B12); promptfoo `synth://` nadal echo (B3); dwie liczby kosztu w kartce `/po-plastrze` (B11).
