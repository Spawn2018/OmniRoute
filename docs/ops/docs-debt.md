...
- **151.0 leftover:** `trip` DONE 152.0. `driver2` DONE 212.0. `route_label` DONE 214.0. km DONE 256.0/257.0. `party_id` DONE 258.0. `/fleet` DONE 259.0. `capacity_kg` DONE 589.0. `capacity_ldm` DONE 590.0. `capacity_m3` DONE 591.0. `document_expiries` DONE 592.0. `inventory_no` DONE 593.0. Zostaje T2c: flagi ADR / telefon / karta; HW. Tu tylko katalog pojazd/kierowca/naczepa.
