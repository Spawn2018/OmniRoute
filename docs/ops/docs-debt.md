...
- **9.0 plan — leftover świadomy (dlaczego nie w pierwszym katalogu):** `network_membership` / numer członkowski; `network_member` + kontakty (katalog agentów); `network_member_link` + RapidFuzz; import Excel/PDF z portalu sieci; ranking M-13; RFQ M-30. Scraping WCA zakazany (sui generis).
