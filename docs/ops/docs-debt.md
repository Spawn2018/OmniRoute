...
- **263.0 leftover:** instancja `task` (FK shipment/trip/stop, assignee, status); matching SQL `applies_when`; outbox/async T5. Tu tylko HITL `task_template` kod + warunek jako dane. Nie worker.
