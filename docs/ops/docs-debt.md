...
- **196.0 leftover:** N3 countdown remaining + szkic `charge`; `demurrage_free_days` na `container`; live HTTP armatora. Blank sailing / congestion / Gate OS = **565.0** `blank_sailing_mark`. Tu tylko `free_time_clock` HITL `clock_kind` + `free_days` + `source_ref`. Nie odliczanie. Nie marża.
