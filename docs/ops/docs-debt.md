...
- **162.0 leftover:** D9b `shipment_ref` HITL DONE 204.0. D9c katalog `network_print_requirement` DONE 626.0. Zostaje: 409 wyjazdu (D9c; nie C8); PDF/ZPL/QR bitmap (D9d); skan HITL bez kodu Omni (D9e); `branding_ref` / `pdf`/`zpl` (D9f). Tu katalog `own_label`/`cmr` + `html_print` + numer HITL + wymóg sieci. Nie D8.
