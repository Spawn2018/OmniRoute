...
- **272.0 leftover:** `blob_ciphertext` / `wrapped_dek` / `tenant_contract_kek`; super-admin = bytea; CI1 `sla_clause`; CT7 p44/FourKites/Shippeo live (TO_VERIFY umowa). Tu tylko `customer_contract` HITL nagłówek kod + etykiety + `source_ref`. Nie treść. Nie KMS.
