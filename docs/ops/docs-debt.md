...
- **24.0 leftover — silnik różnicy Decimal (dlaczego nie w tym plasterze):** `quotation.amount − channel.amount` w JS zakazane (LLM nie liczy; nie drugi `margin()`). Tabela spread / won-lost = zgadywanie schematu. IMAP = M-32. `just dup` 2,67%.
