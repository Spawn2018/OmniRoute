...
- **14.0 leftover — auto-scoring / biuro / limit (dlaczego nie w tym plasterze):** brak `legal_form` i PESEL; AI Act zakazuje scoringu osoby. HTTP BIK/KRD = sekrety tenanta (leftover HC). `party.credit_limit` zostaje ręczną parą Decimal z 5.0. M-15 VDF = następny Plan, LLM nie liczy. `just dup` 2,76%.
