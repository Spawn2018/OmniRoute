...
- **196.0 leftover:** N3 countdown remaining + szkic `charge`; `detention_free_days` / `mixed_dd_days` na `container`; live HTTP armatora. Blank sailing / congestion / Gate OS = **565.0** `blank_sailing_mark`. `demurrage_free_days` = **566.0**. Tu tylko `free_time_clock` HITL `clock_kind` + `free_days` + `source_ref`. Nie odliczanie. Nie marża.
- **566.0 leftover:** `detention_free_days` · `mixed_dd_days` · N3 countdown · szkic charge · live armator. Tu tylko `demurrage_free_days` HITL (dni, nie godzin).
