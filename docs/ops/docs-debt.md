...
- **4.0 środowisko (dlaczego nie w tym plasterze):** instalator EDB odrzuca argumenty z winget (exit 1, zero własnego logu) także po elewacji — PG 16 stoi jako klaster przenośny `tools/pg16` + `tools/pgdata` (gitignore), start `pg_ctl`. `scripts/dev-native.ps1` nadal zna tylko instalację systemową w `Program Files`; dopisanie ścieżki przenośnej to osobna zmiana ops, nie plaster katalogu portów. Dysk `C:` = 0 B wolnego (stan zastany, nie z tego plastra) — `npx` pada ENOSPC, `jscpd` odpalany z `node_modules/.bin`.
...
- **U-catalog-parts:** `components/catalog/catalog-parts.tsx` (nagłówek, banner błędu, notka sesji, formularz resolve) używany przez `/charge-codes` i `/ports`. Pozostałe katalogi (`rate-lines`, `charges`, `quotations`, `organization-settings`) nadal mają własne kopie — przepięcie w slocie `refaktor-pass`, nie w plastrze domenowym.
