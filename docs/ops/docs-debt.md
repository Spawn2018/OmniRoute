...
- **238.0 leftover:** leftover T3 VGM bundle (`vgm_kg` / `vgm_method` / `vgm_cutoff_at`); `last_survey_at`; `carrier_party_id` FK; PIN; booking. Tu tylko `cfs_cutoff_at` HITL. Nie live HTTP. Nie PIN.
