...
- **183.0 leftover:** CO₂ (G2.14–G2.18). Tu tylko `tender_ted_notice` HITL numer TED + `source_ref`. Nie scrape. Nie live HTTP. Nie kwota. Nie auto-award.
- **182.0 leftover:** TED DONE 183.0. Zostaje CO₂ (G2.14–G2.18). Tu tylko `tender_award_review` HITL countersign/challenge + `source_ref`. Nie mutacja `tender.status`. Nie win/loss. Nie dwa user_id. Nie kwota. Nie auto-award.
- **181.0 leftover:** four-eyes DONE 182.0. TED DONE 183.0. Zostaje CO₂ (G2.14–G2.18). Tu tylko `tender_bid_stance` HITL bid/no-bid + `source_ref`. Nie mutacja `tender.status`. Nie win/loss. Nie kwota. Nie auto-award.
...
- **162.0 leftover:** `shipment_ref` na `shipment` (D9b; przed QR/F10); `network_print_requirement` / 409 (D9c; nie C8); PDF/ZPL/QR bitmap (D9d); skan HITL bez kodu Omni (D9e); `branding_ref` / `pdf`/`zpl` (D9f). Tu tylko katalog `own_label`/`cmr` + `html_print`. Nie D8.
