...
| Data | Co |
|---|---|
| 2026-09-24 | **648.0** leftover EXP1 `rate_line.index_id` FSC HITL zamknięty: opcjonalny tekst pin indeksu na `rate_line` (`CONFIRMED`). FK fuel_index / float / mnożenie FSC nadal `REJECTED`. Następny = EXP1 `charge_code` waiting/no_show/diversion/stamp (**649.0**, `REQUIREMENT`). Marża zostaje na `charge` (`CONFIRMED`). |
...
| 2026-09-23 | **642.0** leftover EXP0.9 packing group HITL: kolumna `packing_group` (`I`\|`II`\|`III`) na `dangerous_good` — nie live IMO, nie LLM klasy (`REQUIREMENT`). Grupy 1.xA nadal `REJECTED`. Marża zostaje na `charge` (`CONFIRMED`). |
| 2026-09-23 | **641.0** leftover P4c bind stance HITL zamknięty: katalog `local_charge_bind_mark` (`charge`\|`quote`\|`other`) (`CONFIRMED`). FK UUID / matching SQL / warning-jako-fakt nadal `REJECTED`. Marża zostaje na `charge` (`CONFIRMED`). |
| 2026-09-23 | **641.0** leftover P4c bind stance HITL: katalog `local_charge_bind_mark` (`charge`\|`quote`\|`other`) — nie FK UUID do charge/quote (`REQUIREMENT`). Matching SQL / warning-jako-fakt `REJECTED`. Marża zostaje na `charge` (`CONFIRMED`). |
