...
<!-- os-status:start -->
**Stan (z CURRENT.md):** **444.0** AI2.1 `version_score` — średnie MAE/CRPS per `model_version` **Następny:** **445.0** widok: `created_on` + `pair_count` + średnie MAE/CRPS per `model_version` i dzień UTC. Nie detektor. Nie auto-champion.
<!-- os-status:end -->
