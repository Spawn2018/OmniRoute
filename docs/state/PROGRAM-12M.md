...
<!-- os-status:start -->
**Stan (z CURRENT.md):** **149.0** U4 organization_calendar **Następny:** **T1** `stop` + `stop_group` + timezone + `eta_physical`/`eta_legal` + EXP1 → T → U3 → D → P → G2.0–G2.23 → F → C → V → W → S53 → X → WA1 → Plat → Demo-1 → CT → CI9–CI8 → G → EXP → Mob → K0. Named parks parked — `/noc` pomija aż CURRENT wskaże S53. Nic z pinu 2026-09-08c nie wypada. Nie zgaduj 71–212.
<!-- os-status:end -->
