...
<!-- os-status:start -->
**Stan (z CURRENT.md):** **272.0** leftover HITL `customer_contract` nagłówek **Następny:** plaster **273.0** leftover CI9 opaque `blob_ciphertext` (HITL fixture bytes present/absent, nie szyfr; park KMS/unwrap/KEK). Delta zaakceptowana `/noc` — wolno `/plaster`. Leftover HITL/SQL = praca. Park = live HTTP bez testu albo sekretu, nie skip pola. Live M-02 konsument / Auth0 I1/I2 / portale / giełda live tylko gdy ten ID jest bieżącym Q. P6c auto-award zakaz. Nic z pinu nie wypada. Nie zgaduj 71–273.
<!-- os-status:end -->
