...
**Ostatni plaster:** **262.0** leftover T4b SQL `charge`  
**Etap:** Kod  
**Noc:** `/noc 17` do 2026-09-10T17:00+02. Umowa: [nocna-zmiana.md](../ops/nocna-zmiana.md).  
**Następny:** **263.0** leftover T5 `task_template` → outbox T5 → `plan_snapshot`/kółka → F9 Optima fixture → T8 → S53 Auth0 → portale/diada → CT7/CI9/reszta pinu 2026-09-08c. Leftover HITL/SQL = praca. Park = live HTTP bez testu albo sekretu, nie skip pola. Live M-02 konsument / Auth0 / portale tylko gdy ten ID jest bieżącym Q. P6c auto-award zakaz. Nic z pinu nie wypada. Nie zgaduj 71–263.  
...
**2026-09-08c:** pin całości rozmowy Luki i ulepszenia. Delta **263.0** zaakceptowana (`/noc`). Wolno `/plaster`. W **tym** plasterze: HITL `task_template` (`template_code` + `applies_when` jako dane + `source_ref`), nie matching SQL, nie instancja `task`, nie outbox, nie live VIES, nie live KSeF, nie PIN, nie auto-award, nie T-SQL. Kolejne Q z osi jadą po zamknięciu 263.0 — `/noc` ich nie pomija. LLM nie liczy.
