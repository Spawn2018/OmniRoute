...
**Spec (jedna na sesję):** [docs/spec/operator-notice.md](../spec/operator-notice.md) — nie otwieraj `mail-client.md` jako kolejki 27.0.

**Kanon:** [docs/PLAN-REALIZACJA.md](../PLAN-REALIZACJA.md) — jedyny plan, § Kolejka realizacji.

**Uczciwość:** 27.0 odczyt istniejącej pracy, nie kolejka push, nie sekrety. Nie nadpisywać `charge`. HITL zostaje. ExtractionService nie importuje quotations. LLM nie liczy.

**Środowisko lokalne:** `just`, `lint-imports`, `psql`, `pg_ctl` są w PATH. Przed pracą podnieś dwie rzeczy: `pg_ctl -D tools\pgdata -o "-p 5432" start` oraz `tools\openfga\openfga.exe run`. Potem `just gate` i `just test` działają bez ustawiania zmiennych. PG 16 to klaster przenośny w `tools/pg16` — instalator EDB przez winget nie przechodzi (exit 1).

**2026-09-02:** Plan 27.0 na kolejce. Następny: `/plaster` 27.0 (nie nowa rozmowa w `/noc`).
