...
**Ostatni plaster:** **251.0** leftover T1 EXP1 `seal_out`  
**Etap:** Kod  
**Noc:** `/noc 17` do 2026-09-10T17:00+02. Umowa: [nocna-zmiana.md](../ops/nocna-zmiana.md).  
**Następny:** **252.0** leftover T1 EXP1 `appointment_ref` → waiting/POD → km/`/fleet` → `consignment` → T6 mapa → SQL `charge` → lookup/KSeF TE → outbox T5 → `plan_snapshot`/kółka → F9 Optima fixture → T8 → S53 Auth0 → portale/diada → CT7/CI9/reszta pinu 2026-09-08c. Leftover HITL/SQL = praca. Park = live HTTP bez testu albo sekretu, nie skip pola. Live M-02 konsument / Auth0 / portale tylko gdy ten ID jest bieżącym Q. P6c auto-award zakaz. Nic z pinu nie wypada. Nie zgaduj 71–252.  
...
**2026-09-08c:** pin całości rozmowy Luki i ulepszenia. Delta **252.0** zaakceptowana (`/noc`). Wolno `/plaster`. W **tym** plasterze: `appointment_ref` HITL na `stop`, nie D3 duplicate, nie `appointment_status`, nie `no_show_at`, nie waiting, nie POD, nie PIN, nie live HTTP, nie km, nie `/fleet`, nie SQL na `charge`, nie N1, nie T5, nie mapa, nie auto-award, nie T-SQL. Kolejne Q z osi jadą po zamknięciu 252.0 — `/noc` ich nie pomija. LLM nie liczy.
