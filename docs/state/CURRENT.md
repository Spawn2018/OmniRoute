...
**Spec (jedna na sesję):** [docs/deltas/open/266.0-circle-sim.md](../deltas/open/266.0-circle-sim.md). Oś: [PLAN-REALIZACJA.md](../PLAN-REALIZACJA.md) § Oś leftoverów `/noc`. Karty `karty-pol-fala-*.md`. Ten plaster: leftover HITL `circle_sim` jako dane. Nie RAG na stawkach / umowach CI. Nie pgvector. Nie T8 live API. Nie drugi czat. Nie N8. Nie live VIES. Nie auto-award. Nie scrape. Nie Citizen API. Nie silnik ≥500k. Nie km ładowny. Nie kalkulator kg. Nie live PUESC/KSeF. Nie 409 na shipment. Nie CAMT. Nie silnik CRPS. Nie Open-Meteo HTTP. Nie myto. Nie countdown D&D. Nie live GPS. Nie sekrety adaptera. Nie scoring osoby. Nie silnik EBITDA. Nie fizyka twinów. ExtractionService nie importuje tenders/rates. LLM nie sumuje.
...
**2026-09-08c:** pin całości rozmowy Luki i ulepszenia. Delta **266.0** zaakceptowana (`/noc`). Wolno `/plaster`. W **tym** plasterze: tabela `circle_sim` (HITL kod + para UN/LOCODE unload/load + `source_ref`), nie silnik ≥500k, nie km, nie P overlap, nie PIN, nie live HTTP, nie T-SQL. Kolejne Q z osi jadą po zamknięciu 266.0 — `/noc` ich nie pomija. LLM nie liczy.
