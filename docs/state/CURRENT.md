...
M-02 **fundament 79.0** (konsument = krok osi T5, nie skip nocy). S53 HITL `idp_connector` **270.0**. S55 HITL `exchange_connector` **271.0**. CI9 HITL `customer_contract` nagłówek **272.0**. CI9 opaque BYTEA **273.0** (present/absent, nie szyfr). CI9 znacznik owijki **274.0** (nie klucz, nie szyfr). CT7 HITL `visibility_connector` **275.0** (token `p44`, nie live). CT1 HITL `purchase_order` nagłówek **276.0**. CT1 HITL `po_line` **277.0** (nie ASN). S21 live HTTP = park live. S50 = T2 (`resource` 151.0, `trip` 152.0; leftover T2c `driver2` = 212.0; leftover T2 `route_label` = 214.0; leftover T2c km = 256.0/257.0; leftover `party` = 258.0; leftover `/fleet` = 259.0). Exit Wave FE **nie** claim.

**Spec (jedna na sesję):** brak — następne Q to leftover CT1 HITL ASN / reszta pinu. Komenda `/plan-modul`. Oś: [PLAN-REALIZACJA.md](../PLAN-REALIZACJA.md) § Oś leftoverów `/noc`. 277.0 = HITL linia `po_line`, nie ASN. 276.0 = HITL nagłówek `purchase_order`. 275.0 = HITL katalog `p44` fixture, nie live track. 272.0 = nagłówek; 273.0 = opaque blob present/absent, nie szyfr. 274.0 = znacznik, nie klucz. Nie auto `shipment`. Nie KMS. Nie unwrap. Nie `wrapped_dek`. Nie Fernet/AES. Nie materiał klucza. Nie CT7 live. Nie CI1 extract. Nie `sla_clause`. Nie super-admin plaintext (nie ma decode). Nie G2.22 P/lista. Nie RAG na stawkach / umowach CI. Nie pgvector. Nie T8 live API. Nie drugi czat. Nie N8. Nie live VIES. Nie auto-award. Nie scrape. Nie Citizen API. Nie silnik ≥500k. Nie kalkulator kg. Nie live PUESC/KSeF. Nie 409 na shipment. Nie CAMT. Nie silnik CRPS. Nie Open-Meteo HTTP. Nie myto. Nie countdown D&D. Nie live GPS. Nie sekrety adaptera. Nie scoring osoby. Nie silnik EBITDA. Nie fizyka twinów. ExtractionService nie importuje tenders/rates i **nie** widzi umów CI ani znacznika KEK. LLM nie sumuje.
...
**2026-09-08c:** pin całości rozmowy Luki i ulepszenia. **277.0** zamknięty (`/noc`) — HITL `po_line`, nie ASN. 276.0 = nagłówek. 275.0 = fixture `p44`, nie live. 274.0 = znacznik, nie klucz. 273.0 nadal present/absent, nie szyfr. Następne Q = plan leftover CT1 HITL ASN / reszta pinu. CT8 AIS = park live. Nie CI1. `/noc` nie pomija osi. LLM nie liczy.
