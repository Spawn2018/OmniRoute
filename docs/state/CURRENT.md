...
**Spec (jedna na sesję):** leftover parked — nie otwieraj U3c pule / P4c / P1c–d / D9c–f / live HTTP / auto-award / circle_sim / 409 na shipment. Oś: karty `karty-pol-fala-*.md`. Nie RAG na stawkach / umowach CI. Nie pgvector. Nie T8 live API. Nie drugi czat. Nie N8 koalescencja. Nie mapa. ExtractionService nie importuje tenders/rates. LLM nie sumuje.
...
**2026-09-08c:** pin całości rozmowy Luki i ulepszenia. Delta **209.0** zamknięta (`/noc`). Kolejka żywa = leftover T4 `parent_shipment_id` (HITL FK, nie SQL na `charge`). Named parks parked. Nie pula M-03. Nie IATA live. Nie ocean_bill. Nie warning-jako-fakt. Nie P3b na `charge`. Nie live HTTP. Nie auto-award. Nie parser WHEN/IF. Nie T-SQL. Nie QR. Nie PDF. Nie 409 na shipment. Nie scrape. Nie kalkulator kg. Nie circle_sim. Nie kwota na skonto. Nie CAMT. Nie silnik CRPS. Nie Open-Meteo. Nie myto. Nie countdown D&D. Nie live GPS. Nie sekrety adaptera. Nie scoring osoby. Nie silnik EBITDA. Nie fizyka twinów. Nie N8. Nie drugi czat. Nie RAG na stawkach. LLM nie liczy.
