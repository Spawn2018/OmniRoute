...
**Następny:** kod **271.0** leftover HITL `exchange_connector` Trans.eu fixture (S55). Po zamknięciu: CT7/CI9/reszta pinu 2026-09-08c. Leftover HITL/SQL = praca. Park = live HTTP bez testu albo sekretu, nie skip pola. Live M-02 konsument / Auth0 I1/I2 / portale tylko gdy ten ID jest bieżącym Q. P6c auto-award zakaz. Nic z pinu nie wypada. Nie zgaduj 71–271.  
M-02 **fundament 79.0** (konsument = krok osi T5, nie skip nocy). S53 HITL `idp_connector` **270.0**. S55 HITL `exchange_connector` = ten Q. S21 live HTTP = park live. S50 = T2 (`resource` 151.0, `trip` 152.0; leftover T2c `driver2` = 212.0; leftover T2 `route_label` = 214.0; leftover T2c km = 256.0/257.0; leftover `party` = 258.0; leftover `/fleet` = 259.0). Exit Wave FE **nie** claim.

**Spec (jedna na sesję):** [docs/deltas/open/271.0-exchange-connector.md](../deltas/open/271.0-exchange-connector.md) — leftover S55 HITL konektor giełdy fixture `trans_eu`. Delta zaakceptowana (`/noc`). Wolno `/plaster`. Oś: [PLAN-REALIZACJA.md](../PLAN-REALIZACJA.md) § Oś leftoverów `/noc`. Nie S53 ponownie. Nie live giełda. Nie X1–X5 SPA. Nie OAuth2. Nie G2.22 P/lista. Nie RAG na stawkach / umowach CI. Nie pgvector. Nie T8 live API. Nie drugi czat. Nie N8. Nie live VIES. Nie auto-award. Nie scrape. Nie Citizen API. Nie silnik ≥500k. Nie kalkulator kg. Nie live PUESC/KSeF. Nie 409 na shipment. Nie CAMT. Nie silnik CRPS. Nie Open-Meteo HTTP. Nie myto. Nie countdown D&D. Nie live GPS. Nie sekrety adaptera. Nie scoring osoby. Nie silnik EBITDA. Nie fizyka twinów. ExtractionService nie importuje tenders/rates. LLM nie sumuje.
...
**2026-09-08c:** pin całości rozmowy Luki i ulepszenia. **270.0** zamknięty (`/noc`) — HITL `idp_connector` token `auth0`, JWT hello zostaje, zero live Auth0. Plan **271.0** zaakceptowany (`/noc`) — HITL `exchange_connector` token `trans_eu`, zero live giełdy, zero SPA. `/noc` nie pomija osi. LLM nie liczy.
