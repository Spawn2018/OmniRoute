# Bieżący focus

**Faza:** Oś pinu — po **638.0** (leftover P5b variance stance HITL)

**Repo:** https://github.com/Spawn2018/OmniRoute

**Ostatni plaster:** **638.0** leftover P5b `trip_variance_mark` (gate success)

**Etap:** Plan **639.0** zaakceptowany (`docs/deltas/open/639.0-margin-match-mark.md`). Wolno `/plaster`.

**Noc:** `/noc 17` do **2026-09-23T17:00:00+02:00**

**Miejsce pracy:** produkt = to repo + GitHub.

**Następny:** **639.0** leftover N6 matching stance HITL (`margin_match_mark` stance match|hold|waive|other; zero matching SQL lane / auto charge). D9d PDF-QR / D9e skan / D9f pdf-zpl live / matching T5 / FK kontekstu / T8 live / D6b / D7b / fx×FV / auto-copy U1 / similar SQL / F2b / auto-resync salda / live 409 / live poll / Expo BR2.3 / warning-jako-fakt / N8 / T6 live / P5c actual-from-charge = park `/noc`. **588.0** SH-R16-4 UXCL = park `/noc`.

**Park / czeka w PLAN:** D9d · D9e · D9f pdf-zpl live · live 409 · live poll · Expo · fx×FV · matching T5 · FK kontekstu · T8 live · D6b · D7b · auto-copy U1 · similar SQL · F2b · auto-resync salda · N8 · T6 live · P5c · HW · SH-R16-4 UXCL · AI3 · G0-SH · N3 ·

**Spec (jedyna na sesję produktu):** [docs/deltas/open/639.0-margin-match-mark.md](../deltas/open/639.0-margin-match-mark.md)

**Kanon:** [PLAN-REALIZACJA.md](../PLAN-REALIZACJA.md). Wizja: [VISION.md](../VISION.md).

**Uczciwość:** HITL. LLM nie liczy. `charge` = marża.

**Środowisko lokalne:** `just`, `lint-imports`, `psql`, `pg_ctl` w PATH. PG 16: `tools\pg16`.

**2026-09-23:** Plan **639.0** leftover N6 matching stance HITL (`/noc 17`). 638.0 gate success.
**2026-09-23:** Plaster **638.0** leftover P5b `trip_variance_mark` zamknięty (gate success). `/noc 17`.
**2026-09-23:** Plan **638.0** leftover P5b variance stance HITL (`/noc 17`). 637.0 gate success.
**2026-09-23:** Plaster **637.0** leftover 543 handover bind HITL zamknięty (gate success). `/noc 17`.
**2026-09-23:** Plan **637.0** leftover 543 handover bind HITL (`/noc 17`). 636.0 gate success.
**2026-09-23:** Plaster **636.0** leftover D9f `output_kind` pdf|zpl HITL zamknięty (gate success). `/noc 17`.
**2026-09-23:** Plan **636.0** leftover D9f `output_kind` pdf|zpl HITL (`/noc 17`). 635.0 gate success.
**2026-09-23:** Plaster **635.0** leftover P4c `local_charge_warning_mark` zamknięty (gate success). `/noc 7`.
