...
| M-55 | Reklamacja ładunku | 191.0 `cargo_claim` | **ukończony (S51 + EXP0.8 HITL + 645.0 evidence)** · OS&D + terminy CMR + evidence bool; nie kwota; nie silnik 7/21/365 |
...
| M-71 | Szyna decyzji operatora | 74.0 `operator_decision` · 77.0 lock · 121.0 `changed` · **544.0** `margin_floor` | **ukończony (leftover S11b)** · pending → accept/changed/reject + `lock_version`; nie extract 1.3; nie send |
...
| BR6.1 | Korytarz sprzedażowy HITL | 460.0 `sales_lane` · 491.0 UN/LOCODE · 492.0 `volume_label` · 497.0 `sales_bind_mark` | **ukończony (HITL)** · leftover HubSpot live park |
| BR6.2 | Tryb przetargu załadowcy HITL | 461.0 · 493.0 · 494.0 · 498.0 `shipper_bind_mark` · 499.0 `shipper_award_mark` | **ukończony (HITL)** · leftover Alpega live / auto-award SQL |
...
| F10 | Faktura zakupu HITL | **549.0**–**551.0** `purchase_invoice` · `invoice_match_mark` · `invoice_alloc_mark` | **ukończony (HITL)** · leftover candidate/ranking/allocation z kwotą |
...
| CI4 | Prognoza opóźnienia HITL | 314.0 `delay_forecast` | **ukończony (HITL)** · p_late Decimal; leftover wróżba punktowa / GPS |
| CI6 | Opcja naprawy HITL | 315.0 `remediation_option` | **ukończony (HITL)** · kind bez kwoty; leftover repair_cost / S11 |
| CI6 | Scenariusz skutku HITL | 316.0 `impact_scenario` | **ukończony (HITL)** · chain_label; leftover EBITDA SQL |
...
| CI5 | Znacznik kary HITL | 321.0 `penalty_mark` | **ukończony (HITL)** · breach_kind; leftover kara SQL |
