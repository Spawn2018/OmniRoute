...
| **O6** | `extraction_draft.draft_kind=carrier_quote` → HITL → `channel_quote` + `answered` | Plan → plaster | zamknięty leftover HITL ([531.0](deltas/archived/531.0-carrier-quote-hitl.md); leftover `quote_recorded` przy accept → [534.0](deltas/open/534.0-quote-recorded-on-accept.md)) | ExtractionService nie zapisuje stawek |
