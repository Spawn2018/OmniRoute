...
| T5 | task engine (warunki = dane) | zamknięty HITL szablon ([263.0](deltas/archived/263.0-task-template.md); leftover outbox kind ([264.0](deltas/archived/264.0-outbox-task-template.md)); leftover instancja `task` — plan 616.0; leftover matching SQL / konsument) | po T2 | async po konsumencie outboxa (leftover M-02) |
...
| T7 | `fx_rate_basis` SQL; kalendarz U4 | zamknięty HITL ([211.0](deltas/archived/211.0-fx-rate-basis.md); leftover T7b override/`charge` [615.0](deltas/archived/615.0-charge-fx-rate.md); leftover T7c U4 D-1) | po T1+U4 | LLM/JS nie liczą |
