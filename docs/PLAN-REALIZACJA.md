...
| ID | Co | Tryb | Status | Poza |
|---|---|---|---|---|
| T1 | `stop` + `stop_group` + timezone + `eta_physical`/`eta_legal` + EXP1 (awizacja, plomba, waga, waiting, POD quality) | zamknięty punkt ([150.0](deltas/archived/150.0-stop.md)); ETA HITL ([194.0](deltas/archived/194.0-stop-eta.md)); leftover `stop_group` / EXP1 | po U4 | nie mapa |
...
| V2 / V2b | ETA **dwa czasy** `eta_physical`/`eta_legal` + pogoda; myto → `charge`+`source_ref` | ETA HITL zamknięte ([194.0](deltas/archived/194.0-stop-eta.md)); delta [195.0](deltas/open/195.0-weather-observation.md) HITL pogoda bez HTTP; leftover Open-Meteo / myto | brak taryfy = warning |
