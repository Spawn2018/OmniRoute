...
<!-- os-status:start -->
**Następny (zablokowany):** P0 leftover `charge.source_ref` → O0 → M10-1/M10-2 → B0a → O1–O3 → I0/U2 → O4–O8 → N5 → U1+U5 → I1–I4 → U4 → T → U3 → D → P → G2.0–G2.23 → F → C → V → W → S53 → X → WA1 → Plat → Demo-1 → CT → CI9–CI8 → G → EXP → Mob → K0. Named parks parked — `/noc` pomija aż CURRENT wskaże S53. Nic z pinu 2026-09-08c nie wypada. Nie zgaduj 71–212.
<!-- os-status:end -->
...
| po parks | **P0** → pełna oś pinu **2026-09-08c** (O/I/U/T/D/P/G2/F/C/V/W/X/Plat/CT/CI/G/EXP) | Plan → plaster | **następny** |
...
|---|---|---|---|---|
| **P0** | `charge.source_ref` (nullable stare fixture; obowiązkowe na nowym INSERT) | Plan → plaster | **następny** | HC-05. Karta [karty-pol-fala-p.md](analysis/karty-pol-fala-p.md). Nie zgadywać numeru migracji |
| **O0** | `network_member.party_id` FK tenanta | Plan → plaster | po P0 | Bez tego brak historii agenta. 409 na ranking bez wiązania |
| **M10-1** | Dedup NIP/VAT-EU/EORI/DUNS + wymóg ID biznesowego | Plan → plaster | po O0 | 409 z linkiem. Zakaz B2C bez NIP |
...
| **O5** | `party_lane_scorecard` + szablon podpowiedzi z SQL | Plan → plaster | po O3 | `sample_size=0` i tak ma tekst. Nie LLM. Nie scoring osoby |
...
|---|---|---|---|---|
| **I0 / U2** | `quotation.incoterm` + `trade_side` + `named_place` + wersja 2020/2010 | Plan → plaster | po O3, przed O4 | DAP bez miejsca = 409; nie cytat ICC |
| **U1** | `field_carry_forward` oferta→zlecenie→booking→FV; diff przy POST | Plan → plaster | po O8, przed I1 | mutacja = nowy wiersz / `superseded_by` |
| **U5** | `document_checklist_rule` (incoterm×side×mode) + `blocks_dispatch` | Plan → plaster | z U1 | ≠ C8 polisa podwykonawcy |
...
| T7 | `fx_rate_basis` SQL; kalendarz U4 | Plan → plaster | po T1+U4 | LLM/JS nie liczą |
| T8 | slot capability + godziny terminalu N4; **confirmed tylko z API** | Plan → plaster | po T3 | nie gwarancja prawna; nie Selenium |

**B0b** (po T2): `prediction_ledger`, `plan_snapshot`, indeks paliwa, TT z actuals — karta [karty-pol-fala-v.md](analysis/karty-pol-fala-v.md).
...
| ID | Co | Status | Uwagi |
|---|---|---|---|
| D1–D7 | linie, paczka, cross-dock, COD, cennik LTL, LCL/HBL, palety Chep/LPR (EXP2.17) | po U3+T2 | karta [karty-pol-fala-d.md](analysis/karty-pol-fala-d.md) |
...
| ID | Co | Status | Uwagi |
|---|---|---|---|
| P1–P3 | rate card / szablony / FSC + indeks BAF/CAF (EXP1) + A11 nowy wiersz | po T7 + M-18 | dane + SQL; nie T-SQL |
| **N6** | `margin_floor` per tenant/lane → 409 albo S11 | po P0+P1 | Decimal; nie LLM |
| P4 | Local Charge Library + THC/ISPS/seal/amendment (EXP4.5) + warning | po O2 | warning ≠ fakt |
| P5 | expected vs actual na `trip` | po T2 | nie druga marża |
| P6 | tender quotes (buy) | po M-25 | nie auto-award; klej G2 |
...
| ID | Co | Status | Poza |
|---|---|---|---|
| G2.0–G2.18 | loty, rundy, data room+NDA, extract RFP HITL, matryca Excel (LLM mapuje kolumny, kwoty z P), playbook, prospecting HITL, bid/no-bid, award four-eyes, win/loss, TED capability, CO₂, consortium | po P6+O4+B0b | auto-award; scrape kontaktów |
...
| F2–F4 | skonto, noty, CAMT+HITL + period lock N13 | po F1 | biała lista przy przelewie |
...
| ID | Co | Status | Uwagi |
|---|---|---|---|
| C1 / C6–C8 | SENT, BDO, `monitoring_scheme`, `party_document` 409 + KREPTD C8 | po T2 | karta [karty-pol-fala-c.md](analysis/karty-pol-fala-c.md) |
| C2 | AIS/AES/Intrastat | TO_VERIFY PUESC | |
| C3–C5 / C9 | lookup live, eCMR 2027 (EXP2.18), CO₂+metodyka, Trans.eu snapshot | C9 bez scrapingu opinii | |
| **EXP0.8** | `cargo_claim` deadline CMR 7/21/365 + OS&D | z M-55 pogłębieniem | nie kwota z LLM |
| **EXP0.9** | UN: `adr_tunnel_code` + `segregation_group` | z M-52 | LLM nie nadaje klasy |

### Fala V — predykcje / telematyka / wieża

| ID | Co | Status | Uwagi |
|---|---|---|---|
| V1 | Prediction Ledger + N7 przedział/kalibracja | po B0b | punkt bez CRPS = zakaz |
| V2 / V2b | ETA **dwa czasy** `eta_physical`/`eta_legal` + pogoda; myto → `charge`+`source_ref` | po V1+T2+P0+U4 | brak taryfy = warning |
| V3 | D&D / rollover + zegar N3 + blank sailing EXP2.7 | po T3 | |
...
| ID | Co | Status | Poza |
|---|---|---|---|
| W1 | 8 twinów (pojazd, kierowca, kontener, zlecenie, sieć, plan, urząd, ładunek) | po V | twin ≠ fizyka; kółka = G2.20 |
