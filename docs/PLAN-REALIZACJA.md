...
| **EXP0.8** | `cargo_claim` deadline CMR 7/21/365 + OS&D | zamknięty HITL OS&D + terminy ([191.0](deltas/archived/191.0-cargo-claim-cmr.md)); evidence DONE **645.0** (trzy bool); leftover Deadline Engine / S11 park | nie kwota z LLM |
| **EXP0.9** | UN: `adr_tunnel_code` + `segregation_group` | zamknięty HITL tunel ADR + SG ([192.0](deltas/archived/192.0-dangerous-good-adr.md)); packing group DONE **642.0**; marine pollutant DONE **643.0**; limited quantity DONE **644.0**; leftover live IMO / grupy 1.xA park | LLM nie nadaje klasy |
