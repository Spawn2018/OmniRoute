...
- Create = `pending`, `lock_version` 0. POST decide: `accepted` albo `rejected` + `lock_version`. Unikat pending na `(organization_id, subject_kind, subject_id)`
- Decide = jeden `UPDATE … WHERE pending AND lock_version`. Drugi Akceptuj = konflikt.
- OpenFGA `can_manage_operator_decisions` = member
- UI `/decisions`: lista + pending + Akceptuj / Odrzuć z wersją
