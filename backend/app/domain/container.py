...
def require_container_release_party_id(raw: object) -> UUID | None:
    if raw is None:
        return None
    if type(raw) is not UUID:
        raise InvalidContainer("wskazanie zwolnienia musi być UUID")
    return raw
...
def require_alliance_service(raw: object) -> str | None:
    if raw is None:
        return None
    if type(raw) is not str:
        raise InvalidContainer("alians musi być tekstem")
    token = raw.strip()
    if token == "":
        return None
    if len(token) > _MAX_ALLIANCE:
        raise InvalidContainer("alians za długi")
    return token
...
def require_container_delivery_date(raw: object) -> date | None:
