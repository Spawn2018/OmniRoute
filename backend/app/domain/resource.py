...
def require_vehicle_profile(raw: object) -> str | None:
    if raw is None:
        return None
    if type(raw) is not str:
        raise InvalidResource("profil pojazdu musi być tekstem")
    token = raw.strip()
    if token == "":
        return None
    if len(token) > _MAX_PROFILE:
        raise InvalidResource("profil pojazdu za długi")
    return token
