...
def require_stop_weigh_out_kg(raw: object) -> Decimal | None:
    if raw is None:
        return None
    if type(raw) is str and raw.strip() == "":
        return None
    if isinstance(raw, float) or isinstance(raw, bool):
        raise InvalidStop("waga wyjazdu nie może być float")
    if not isinstance(raw, Decimal | str | int):
        raise InvalidStop("waga wyjazdu musi być liczbą dziesiętną")
    try:
        parsed = raw if isinstance(raw, Decimal) else Decimal(str(raw))
    except InvalidOperation as exc:
        raise InvalidStop("waga wyjazdu musi być liczbą dziesiętną") from exc
    if parsed < 0:
        raise InvalidStop("waga wyjazdu nie może być ujemna")
    return parsed.quantize(_FOUR)
