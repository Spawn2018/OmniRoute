import uuid

from sqlalchemy import CheckConstraint, ForeignKey, Index, String, UniqueConstraint
from sqlalchemy.dialects.postgresql import UUID
from sqlalchemy.orm import Mapped, mapped_column

from app.models.base import Base, TimestampMixin


class InterventionOutcome(Base, TimestampMixin):
    __tablename__ = "intervention_outcome"
    __table_args__ = (
        UniqueConstraint("organization_id", "id", name="uq_intervention_outcome_org_id"),
        UniqueConstraint(
            "organization_id",
            "outcome_code",
            name="uq_intervention_outcome_org_code",
        ),
        UniqueConstraint(
            "organization_id",
            "source_ref",
            name="uq_intervention_outcome_org_source_ref",
        ),
        CheckConstraint(
            "outcome_code ~ '^[a-z][a-z0-9_]{1,31}$'",
            name="ck_intervention_outcome_code",
        ),
        CheckConstraint(
            "result_kind IN ('contained', 'rerouted', 'claimed', 'other')",
            name="ck_intervention_outcome_result_kind",
        ),
        Index("ix_intervention_outcome_organization_id", "organization_id"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True)
    # RLS: wynik interwencji tego tenanta — katalog HITL, nie SQL saved.
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("organization.id", ondelete="RESTRICT"), nullable=False
    )
    outcome_code: Mapped[str] = mapped_column(String(32), nullable=False)
    result_kind: Mapped[str] = mapped_column(String(16), nullable=False)
    source_ref: Mapped[str] = mapped_column(String(256), nullable=False)
