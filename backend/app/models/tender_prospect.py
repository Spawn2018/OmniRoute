...
class TenderProspect(Base, TimestampMixin):
    __tablename__ = "tender_prospect"
    __table_args__ = (
        UniqueConstraint("organization_id", "id", name="uq_tender_prospect_org_id"),
        UniqueConstraint(
            "organization_id",
            "tender_id",
            "party_id",
        ...
        Index("ix_tender_prospect_org_tender", "organization_id", "tender_id"),
        Index("ix_tender_prospect_organization_id", "organization_id"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True)
    # RLS: prospekt i nagłówek jednego tenanta — CHECK kodu nie zastępuje polityki.
    ...
    outreach_code: Mapped[str] = mapped_column(String(32), nullable=False)
