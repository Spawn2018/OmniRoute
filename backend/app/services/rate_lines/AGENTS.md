...
Niemutowalna stawka kupna + `source_ref` + opcjonalny HITL `allotment_teu`. Nie tabela `charge`, nie accept HITL.
...
- mutacja `amount` / `currency` / `source_ref` / `allotment_teu` w miejscu
