from uuid import UUID, uuid4

from sqlalchemy.ext.asyncio import AsyncSession

from app.domain.risk_register_mark import parse_risk_register_mark_row
from app.models.risk_register_mark import RiskRegisterMark
from app.repositories.risk_register_marks.risk_register_mark_repository import (
    RiskRegisterMarkRepository,
)


class RiskRegisterMarkService:
    def __init__(self, session: AsyncSession) -> None:
        self._rows = RiskRegisterMarkRepository(session)

    async def list_marks(self) -> list[RiskRegisterMark]:
        return await self._rows.list_marks()

    async def persist_risk_register_mark(
        self,
        *,
        organization_id: UUID,
        user_id: UUID,
        mark_code: object,
        risk_kind: object,
        source_ref: object,
    ) -> RiskRegisterMark:
        code, kind, origin = parse_risk_register_mark_row(
            mark_code,
            risk_kind,
            source_ref,
        )
        row = RiskRegisterMark(
            id=uuid4(),
            organization_id=organization_id,
            mark_code=code,
            risk_kind=kind,
            source_ref=origin,
            created_by=user_id,
        )
        return await self._rows.add_mark(row)
