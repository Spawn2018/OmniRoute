...
Kontener ISO 6346 per tenant. Opcjonalne `shipment_id`. HITL VGM. HITL `booking_no`. HITL `pin_code` (tekst). HITL `payload_kg` (Decimal, nie kalkulator). HITL `carrier_party_id` (FK, bind w API). HITL `shipment_leg_id` (FK, bind w API). Nie S21.
