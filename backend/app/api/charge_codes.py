from uuid import UUID

from fastapi import APIRouter, Depends, Query, status
from pydantic import BaseModel, ConfigDict, Field
from sqlalchemy.ext.asyncio import AsyncSession

from app.api.deps import get_current_identity, require_permission, require_tenant_session
from app.core.session_token import SessionIdentity
from app.services.charge_codes.charge_code_service import ChargeCodeService

router = APIRouter(prefix="/charge-codes", tags=["charge-codes"])


class ChargeCodeCreate(BaseModel):
    code: str = Field(min_length=1, max_length=32)
    name: str = Field(min_length=1, max_length=128)
    aliases: list[str] = Field(default_factory=list)
    source_ref: str = Field(min_length=1, max_length=512)


class ChargeCodeResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)

    id: UUID
    organization_id: UUID
    code: str
    name: str
    aliases: list[str]
    source_ref: str


@router.get("", response_model=list[ChargeCodeResponse])
async def list_charge_codes(
    _authz: None = Depends(require_permission("can_manage_charge_codes", "organization")),
    session: AsyncSession = Depends(require_tenant_session),
) -> list[ChargeCodeResponse]:
    service = ChargeCodeService(session)
    rows = await service.list_codes()
    return [ChargeCodeResponse.model_validate(row) for row in rows]


@router.get("/resolve", response_model=ChargeCodeResponse)
async def resolve_charge_code(
    token: str = Query(..., min_length=1, max_length=32),
    _authz: None = Depends(require_permission("can_manage_charge_codes", "organization")),
    session: AsyncSession = Depends(require_tenant_session),
) -> ChargeCodeResponse:
    service = ChargeCodeService(session)
    row = await service.resolve(token)
    return ChargeCodeResponse.model_validate(row)


@router.post("", response_model=ChargeCodeResponse, status_code=status.HTTP_201_CREATED)
async def create_charge_code(
    body: ChargeCodeCreate,
    _authz: None = Depends(require_permission("can_manage_charge_codes", "organization")),
    session: AsyncSession = Depends(require_tenant_session),
    identity: SessionIdentity = Depends(get_current_identity),
) -> ChargeCodeResponse:
    service = ChargeCodeService(session)
    row = await service.create_code(
        organization_id=identity.organization_id,
        user_id=identity.user_id,
        code=body.code,
        name=body.name,
        aliases=body.aliases,
        source_ref=body.source_ref,
    )
    await session.commit()
    return ChargeCodeResponse.model_validate(row)


@router.post(
    "/seed",
    response_model=list[ChargeCodeResponse],
    status_code=status.HTTP_201_CREATED,
)
async def seed_charge_codes(
    _authz: None = Depends(require_permission("can_manage_charge_codes", "organization")),
    session: AsyncSession = Depends(require_tenant_session),
    identity: SessionIdentity = Depends(get_current_identity),
) -> list[ChargeCodeResponse]:
    service = ChargeCodeService(session)
    rows = await service.seed_omni_exp1(
        organization_id=identity.organization_id,
        user_id=identity.user_id,
    )
    await session.commit()
    return [ChargeCodeResponse.model_validate(row) for row in rows]
