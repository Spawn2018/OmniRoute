...
@pytest.mark.asyncio
async def test_operational_exception_rls_isolates_tenants(session, two_tenants) -> None:
    ...
    row_a = OperationalException(
        id=uuid4(),
        organization_id=org_a.id,
        shipment_id=ship_a.id,
        exception_kind="noted",
        source_ref="fixture://operational-exception/a",
    ...
    row_b = OperationalException(
        id=uuid4(),
        organization_id=org_b.id,
        shipment_id=ship_b.id,
        exception_kind="blocked",
        source_ref="fixture://operational-exception/b",
    ...
    session.expunge_all()
    await bind_tenant(session, org_a.id)
    visible_a = list((await session.scalars(select(OperationalException))).all())
    assert {row.id for row in visible_a} == {row_a.id}
    foreign_b = await session.scalar(
        select(OperationalException).where(OperationalException.id == row_b.id),
    )
    assert foreign_b is None

    session.expunge_all()
    await bind_tenant(session, org_b.id)
    visible_b = list((await session.scalars(select(OperationalException))).all())
    assert {row.id for row in visible_b} == {row_b.id}


@pytest.mark.integration
@pytest.mark.asyncio
async def test_operational_exception_rejects_foreign_shipment(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    org_b = two_tenants["org_b"]
    user_a = two_tenants["user_a"]
    user_b = two_tenants["user_b"]
    ...
    await bind_tenant(session, org_a.id)
    stolen = OperationalException(
        id=uuid4(),
        organization_id=org_a.id,
        shipment_id=ship_b.id,
        exception_kind="other",
        source_ref="fixture://operational-exception/stolen",
        created_by=user_a.id,
    )
    session.add(stolen)
