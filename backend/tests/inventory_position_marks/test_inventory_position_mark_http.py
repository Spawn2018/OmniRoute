...
def test_migration_279_creates_inventory_position_and_rls() -> None:
    source = (
        _ROOT / "backend/alembic/versions/279_inventory_position_mark.py"
    ).read_text(encoding="utf-8")
    assert 'revision: str = "279_inventory_position_mark"' in source
    assert 'down_revision: str | None = "278_vda_odette_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "inventory_position_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.inventory_position_marks" in forbidden
    assert "app.models.inventory_position_mark" in forbidden


def test_api_types_include_inventory_position_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "InventoryPositionMarkResponse" in source
    assert "InventoryPositionMarkCreate" in source


def test_fga_source_declares_inventory_position_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_inventory_position_marks: member" in source


def test_fga_model_grants_inventory_position_to_member() -> None:
    organization = next(
        definition
        for definition in authorization_model_request().type_definitions
        if definition.type == "organization"
    )
    relation = organization.relations["can_manage_inventory_position_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[InventoryPositionMark] = []
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.inventory_position_marks.InventoryPositionMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "ip_pos_01",
        "stock_kind": "position",
        "source_ref": "fixture://inventory-position-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/inventory-position-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["stock_kind"] == "position"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/inventory-position-marks",
        headers=bearer_auth_headers(),
        json=_payload(stock_kind="live_wms"),
    )
    assert response.status_code == 400
