...
def test_migration_269_creates_peppol_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/269_peppol_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "269_peppol_mark"' in source
    assert 'down_revision: str | None = "268_e_delivery_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "peppol_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.peppol_marks" in forbidden
    assert "app.models.peppol_mark" in forbidden


def test_api_types_include_peppol_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "PeppolMarkResponse" in source
    assert "PeppolMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_peppol_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_peppol_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.peppol_marks.PeppolMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "peppol_01",
        "peppol_kind": "peppol",
        "source_ref": "fixture://peppol-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    assert response.json()["peppol_kind"] == "peppol"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/peppol-marks",
        headers=bearer_auth_headers(),
        json=_payload(peppol_kind="live_peppol"),
    )
    assert response.status_code == 400
