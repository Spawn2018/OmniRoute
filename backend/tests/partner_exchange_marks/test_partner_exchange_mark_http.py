...
def test_migration_273_creates_partner_exchange_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/273_partner_exchange_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "273_partner_exchange_mark"' in source
    assert 'down_revision: str | None = "272_webhook_outbox_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "partner_exchange_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.partner_exchange_marks" in forbidden
    assert "app.models.partner_exchange_mark" in forbidden


def test_api_types_include_partner_exchange_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "PartnerExchangeMarkResponse" in source
    assert "PartnerExchangeMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_partner_exchange_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_partner_exchange_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[PartnerExchangeMark] = []
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.partner_exchange_marks.PartnerExchangeMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "px_partner_01",
        "exchange_kind": "partner",
        "source_ref": "fixture://partner-exchange-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/partner-exchange-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["exchange_kind"] == "partner"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/partner-exchange-marks",
        headers=bearer_auth_headers(),
        json=_payload(exchange_kind="live_exchange"),
    )
    assert response.status_code == 400
