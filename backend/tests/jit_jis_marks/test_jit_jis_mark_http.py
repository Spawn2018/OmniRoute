...
def test_migration_277_creates_jit_jis_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/277_jit_jis_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "277_jit_jis_mark"' in source
    assert 'down_revision: str | None = "276_offboarding_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "jit_jis_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.jit_jis_marks" in forbidden
    assert "app.models.jit_jis_mark" in forbidden


def test_api_types_include_jit_jis_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "JitJisMarkResponse" in source
    assert "JitJisMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_jit_jis_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_jit_jis_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.jit_jis_marks.JitJisMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "jj_jit_01",
        "flow_kind": "jit",
        "source_ref": "fixture://jit-jis-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/jit-jis-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["flow_kind"] == "jit"
