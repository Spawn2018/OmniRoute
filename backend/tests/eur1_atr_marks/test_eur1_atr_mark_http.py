...
def test_migration_283_creates_eur1_atr_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/283_eur1_atr_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "283_eur1_atr_mark"' in source
    assert 'down_revision: str | None = "282_eccn_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "eur1_atr_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.eur1_atr_marks" in forbidden
    assert "app.models.eur1_atr_mark" in forbidden


def test_api_types_include_eur1_atr_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "Eur1AtrMarkResponse" in source
    assert "Eur1AtrMarkCreate" in source


def test_fga_source_declares_eur1_atr_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_eur1_atr_marks: member" in source


def test_fga_model_grants_eur1_atr_to_member() -> None:
    organization = next(
        definition
        for definition in authorization_model_request().type_definitions
        if definition.type == "organization"
    )
    relation = organization.relations["can_manage_eur1_atr_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.eur1_atr_marks.Eur1AtrMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "ea_eur1_01",
        "cert_kind": "eur1",
        "source_ref": "fixture://eur1-atr-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/eur1-atr-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["cert_kind"] == "eur1"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/eur1-atr-marks",
        headers=bearer_auth_headers(),
        json=_payload(cert_kind="live_eur1"),
    )
    assert response.status_code == 400
