...
def test_migration_284_creates_phyto_ata_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/284_phyto_ata_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "284_phyto_ata_mark"' in source
    assert 'down_revision: str | None = "283_eur1_atr_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "phyto_ata_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.phyto_ata_marks" in forbidden
    assert "app.models.phyto_ata_mark" in forbidden


def test_api_types_include_phyto_ata_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "PhytoAtaMarkResponse" in source
    assert "PhytoAtaMarkCreate" in source


def test_fga_source_declares_phyto_ata_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_phyto_ata_marks: member" in source


def test_fga_model_grants_phyto_ata_to_member() -> None:
    organization = next(
        definition
        for definition in authorization_model_request().type_definitions
        if definition.type == "organization"
    )
    relation = organization.relations["can_manage_phyto_ata_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.phyto_ata_marks.PhytoAtaMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "phyto_ata_01",
        "permit_kind": "phyto",
        "source_ref": "fixture://phyto-ata-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/phyto-ata-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["permit_kind"] == "phyto"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/phyto-ata-marks",
        headers=bearer_auth_headers(),
        json=_payload(permit_kind="live_phyto"),
    )
    assert response.status_code == 400
