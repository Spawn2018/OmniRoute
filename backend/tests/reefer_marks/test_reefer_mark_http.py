...
def test_migration_294_creates_reefer_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/294_reefer_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "294_reefer_mark"' in source
    assert 'down_revision: str | None = "293_ocean_alliance_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "reefer_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.reefer_marks" in forbidden
    assert "app.models.reefer_mark" in forbidden


def test_api_types_include_reefer_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "ReeferMarkResponse" in source
    assert "ReeferMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_reefer_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_reefer_marks"]
...
class InMemoryFleetCostDesk:
    ...
    async def persist_reefer_mark(
        ...
        *,
        ...
        code, kind, origin = parse_reefer_mark_row(
            mark_code,
            reefer_kind,
            source_ref,
        )
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.reefer_marks.ReeferMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "rf_ree_01",
        "reefer_kind": "reefer",
        "source_ref": "fixture://reefer-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    assert response.json()["reefer_kind"] == "reefer"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/reefer-marks",
        headers=bearer_auth_headers(),
        json=_payload(reefer_kind="live_reefer"),
    )
    assert response.status_code == 400
