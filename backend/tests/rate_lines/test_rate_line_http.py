...
def test_http_create_with_index_id(rates_client: TestClient) -> None:
    created = rates_client.post(
        "/api/v1/rate-lines",
        headers=bearer_auth_headers(),
        json={
    ...
    assert created.status_code == 201
    assert created.json()["index_id"] == "FSC-Q3-2026"


def test_http_rejects_long_index_id(rates_client: TestClient) -> None:
    response = rates_client.post(
        "/api/v1/rate-lines",
        headers=bearer_auth_headers(),
        json={
