...
def test_migration_272_creates_webhook_outbox_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/272_webhook_outbox_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "272_webhook_outbox_mark"' in source
    assert 'down_revision: str | None = "271_integration_hub_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "webhook_outbox_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.webhook_outbox_marks" in forbidden
    assert "app.models.webhook_outbox_mark" in forbidden


def test_api_types_include_webhook_outbox_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "WebhookOutboxMarkResponse" in source
    assert "WebhookOutboxMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_webhook_outbox_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_webhook_outbox_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[WebhookOutboxMark] = []
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.webhook_outbox_marks.WebhookOutboxMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "wh_hook_01",
        "outbox_kind": "webhook",
        "source_ref": "fixture://webhook-outbox-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/webhook-outbox-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["outbox_kind"] == "webhook"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/webhook-outbox-marks",
        headers=bearer_auth_headers(),
        json=_payload(outbox_kind="live_webhook"),
    )
    assert response.status_code == 400
