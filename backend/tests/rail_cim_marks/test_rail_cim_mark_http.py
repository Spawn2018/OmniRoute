...
def test_migration_292_creates_rail_cim_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/292_rail_cim_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "292_rail_cim_mark"' in source
    assert 'down_revision: str | None = "291_air_ra3_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "rail_cim_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.rail_cim_marks" in forbidden
    assert "app.models.rail_cim_mark" in forbidden


def test_api_types_include_rail_cim_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "RailCimMarkResponse" in source
    assert "RailCimMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_rail_cim_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_rail_cim_marks"]
...
class InMemoryFleetCostDesk:
    ...
    async def persist_rail_cim_mark(
        ...
        *,
        ...
        code, kind, origin = parse_rail_cim_mark_row(
            mark_code,
            rail_kind,
            source_ref,
        )
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.rail_cim_marks.RailCimMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "rcim_uic_01",
        "rail_kind": "uic",
        "source_ref": "fixture://rail-cim-mark/a",
    }
    body.update(extra)
    return body
