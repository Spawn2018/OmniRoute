...
def test_migration_276_creates_offboarding_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/276_offboarding_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "276_offboarding_mark"' in source
    assert 'down_revision: str | None = "275_iso_nis2_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "offboarding_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.offboarding_marks" in forbidden
    assert "app.models.offboarding_mark" in forbidden


def test_api_types_include_offboarding_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "OffboardingMarkResponse" in source
    assert "OffboardingMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_offboarding_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_offboarding_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[OffboardingMark] = []
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.offboarding_marks.OffboardingMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "ob_export_01",
        "offboard_kind": "offboard",
        "source_ref": "fixture://offboarding-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/offboarding-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["offboard_kind"] == "offboard"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/offboarding-marks",
        headers=bearer_auth_headers(),
        json=_payload(offboard_kind="live_wipe"),
    )
    assert response.status_code == 400
