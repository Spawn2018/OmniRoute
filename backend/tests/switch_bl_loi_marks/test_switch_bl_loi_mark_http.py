...
def test_migration_285_creates_switch_bl_loi_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/285_switch_bl_loi_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "285_switch_bl_loi_mark"' in source
    assert 'down_revision: str | None = "284_phyto_ata_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "switch_bl_loi_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.switch_bl_loi_marks" in forbidden
    assert "app.models.switch_bl_loi_mark" in forbidden


def test_api_types_include_switch_bl_loi_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "SwitchBlLoiMarkResponse" in source
    assert "SwitchBlLoiMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_switch_bl_loi_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_switch_bl_loi_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[SwitchBlLoiMark] = []
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.switch_bl_loi_marks.SwitchBlLoiMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "sbl_bl_01",
        "instrument_kind": "bl",
        "source_ref": "fixture://switch-bl-loi-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/switch-bl-loi-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["instrument_kind"] == "bl"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/switch-bl-loi-marks",
        headers=bearer_auth_headers(),
        json=_payload(instrument_kind="live_switch_bl"),
    )
    assert response.status_code == 400
