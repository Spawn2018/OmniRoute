...
def test_migration_290_creates_csrd_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/290_csrd_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "290_csrd_mark"' in source
    assert 'down_revision: str | None = "289_demand_snapshot_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "csrd_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.csrd_marks" in forbidden
    assert "app.models.csrd_mark" in forbidden


def test_api_types_include_csrd_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "CsrdMarkResponse" in source
    assert "CsrdMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_csrd_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_csrd_marks"]
...
class InMemoryFleetCostDesk:
    ...
    async def persist_csrd_mark(
        ...
        *,
        ...
        code, kind, origin = parse_csrd_mark_row(
            mark_code,
            report_kind,
            source_ref,
        )
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.csrd_marks.CsrdMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "csrd_rep_01",
        "report_kind": "csrd",
        "source_ref": "fixture://csrd-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    assert response.json()["report_kind"] == "csrd"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/csrd-marks",
        headers=bearer_auth_headers(),
        json=_payload(report_kind="live_csrd"),
    )
    assert response.status_code == 400
