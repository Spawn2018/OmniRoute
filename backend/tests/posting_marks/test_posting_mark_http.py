...
def test_migration_298_creates_posting_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/298_posting_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "298_posting_mark"' in source
    assert 'down_revision: str | None = "297_multi_manning_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "posting_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.posting_marks" in forbidden
    assert "app.models.posting_mark" in forbidden


def test_api_types_include_posting_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "PostingMarkResponse" in source
    assert "PostingMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_posting_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_posting_marks"]
...
class InMemoryFleetCostDesk:
    ...
    async def persist_posting_mark(
        ...
        *,
        ...
        code, kind, origin = parse_posting_mark_row(
            mark_code,
            posting_kind,
            source_ref,
        )
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.posting_marks.PostingMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "po_post_01",
        "posting_kind": "posting",
        "source_ref": "fixture://posting-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/posting-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["posting_kind"] == "posting"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/posting-marks",
        headers=bearer_auth_headers(),
        json=_payload(posting_kind="live_posting"),
    )
    assert response.status_code == 400
