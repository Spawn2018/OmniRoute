...
@pytest.mark.asyncio
async def test_version_window_averages_two_pairs_same_day(session, two_tenants) -> None:
    ...
    rows = list((await session.scalars(select(VersionWindow))).all())
    assert len(rows) == 1
    assert rows[0].pair_count == 2
    assert rows[0].avg_mae == Decimal("1.0000")
    assert rows[0].avg_crps == Decimal("1.2500")
    assert rows[0].created_on is not None


@pytest.mark.integration
@pytest.mark.asyncio
async def test_version_window_view_isolates_tenants(session, two_tenants) -> None:
    ...
    session.expunge_all()
    await bind_tenant(session, org_a.id)
    visible = list((await session.scalars(select(VersionWindow))).all())
