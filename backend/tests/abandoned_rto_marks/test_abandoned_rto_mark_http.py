...
def test_migration_285_creates_switch_bl_loi_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/286_abandoned_rto_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "286_abandoned_rto_mark"' in source
    assert 'down_revision: str | None = "285_switch_bl_loi_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "abandoned_rto_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.abandoned_rto_marks" in forbidden
    assert "app.models.abandoned_rto_mark" in forbidden


def test_api_types_include_abandoned_rto_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "AbandonedRtoMarkResponse" in source
    assert "AbandonedRtoMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_abandoned_rto_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_abandoned_rto_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[AbandonedRtoMark] = []
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.abandoned_rto_marks.AbandonedRtoMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "aro_rto_01",
        "fate_kind": "abandoned",
        "source_ref": "fixture://abandoned-rto-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/abandoned-rto-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["fate_kind"] == "abandoned"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/abandoned-rto-marks",
        headers=bearer_auth_headers(),
        json=_payload(fate_kind="live_abandoned"),
    )
    assert response.status_code == 400
