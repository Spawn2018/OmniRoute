...
def test_migration_280_creates_fair_share_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/280_fair_share_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "280_fair_share_mark"' in source
    assert 'down_revision: str | None = "279_inventory_position_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "fair_share_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.fair_share_marks" in forbidden
    assert "app.models.fair_share_mark" in forbidden


def test_api_types_include_fair_share_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "FairShareMarkResponse" in source
    assert "FairShareMarkCreate" in source


def test_fga_source_declares_fair_share_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_fair_share_marks: member" in source


def test_fga_model_grants_fair_share_to_member() -> None:
    organization = next(
        definition
        for definition in authorization_model_request().type_definitions
        if definition.type == "organization"
    )
    relation = organization.relations["can_manage_fair_share_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.fair_share_marks.FairShareMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "fs_fair_01",
        "share_kind": "fair",
        "source_ref": "fixture://fair-share-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/fair-share-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["share_kind"] == "fair"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/fair-share-marks",
        headers=bearer_auth_headers(),
        json=_payload(share_kind="live_alloc"),
    )
    assert response.status_code == 400
