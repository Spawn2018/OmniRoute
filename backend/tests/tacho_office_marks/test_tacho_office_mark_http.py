...
def test_migration_299_creates_tacho_office_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/299_tacho_office_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "299_tacho_office_mark"' in source
    assert 'down_revision: str | None = "298_posting_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "tacho_office_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.tacho_office_marks" in forbidden
    assert "app.models.tacho_office_mark" in forbidden


def test_api_types_include_tacho_office_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "TachoOfficeMarkResponse" in source
    assert "TachoOfficeMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_tacho_office_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_tacho_office_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[TachoOfficeMark] = []
    ...
    async def persist_tacho_office_mark(
        ...
        *,
        ...
        code, kind, origin = parse_tacho_office_mark_row(
            mark_code,
            tacho_kind,
            source_ref,
        )
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.tacho_office_marks.TachoOfficeMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "to_off_01",
        "tacho_kind": "office",
        "source_ref": "fixture://tacho-office-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/tacho-office-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["tacho_kind"] == "office"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/tacho-office-marks",
        headers=bearer_auth_headers(),
        json=_payload(tacho_kind="live_tacho"),
    )
    assert response.status_code == 400
