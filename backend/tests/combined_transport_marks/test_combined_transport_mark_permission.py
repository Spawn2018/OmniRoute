...
_DENIED = "Brak uprawnienia can_manage_combined_transport_marks na organization"
_CALLS = (
    ("GET", "/api/v1/combined-transport-marks", None, None),
    (
        "POST",
...
class DenyCombinedTransportAuthz:
    async def check(
        self,
        *,
        user_id: UUID,
...
def test_combined_transport_mark_endpoints_are_forbidden_without_permission(
    method: str,
    path: str,
    params: dict[str, str] | None,
    json_body: dict[str, object] | None,
) -> None:
    set_authz_checker(DenyCombinedTransportAuthz())
...
def test_fga_source_declares_combined_transport_mark_relation() -> None:
