...
_DENIED = "Brak uprawnienia can_manage_allocation_keys na organization"

_CALLS = (
    ("GET", "/api/v1/allocation-keys", None, None),
    (
        "POST",
        "/api/v1/allocation-keys",
        None,
        {"key_code": "lane_direct", "source_ref": "fixture://allocation-key/a"},
    ),
)


class DenyAllocationKeyAuthz:
    async def check(
        self,
        *,
        user_id: UUID,
...
def test_allocation_key_endpoints_are_forbidden_without_permission(
    method: str,
    path: str,
    params: dict[str, str] | None,
    json_body: dict[str, object] | None,
) -> None:
    set_authz_checker(DenyAllocationKeyAuthz())
