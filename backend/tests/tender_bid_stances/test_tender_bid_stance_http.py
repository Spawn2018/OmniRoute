...
class StubStanceDesk:
    def __init__(self, session: object) -> None:
        self._session = session
        self.rows: list[TenderBidStance] = []
...
def catalog_client(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.tender_bid_stances.TenderBidStanceService",
        lambda _s: stances,
    )
    monkeypatch.setattr("app.api.tender_bid_stances.TenderService", lambda _s: boards)
    set_authz_checker(AllowAllAuthz())
    app.dependency_overrides[require_tenant_session] = _fake_tenant_session
    yield TestClient(app), stances, boards
...
def _payload(tender_id: UUID, **overrides: object) -> dict[str, object]:
    body: dict[str, object] = {
        ...
        "stance_code": "bid",
        "source_ref": "fixture://tender-bid-stance/1",
    }
    body.update(overrides)
    return body


def test_http_create_and_list_tender_bid_stance(catalog_client: object) -> None:
    client, _stances, boards = catalog_client
    assert boards.row is not None
    org_id = uuid4()
    headers = bearer_auth_headers(organization_id=org_id)
    created = client.post(
        "/api/v1/tender-bid-stances",
        headers=headers,
    ...
    assert body["stance_code"] == "bid"
    assert "buy_amount" not in body
    listed = client.get("/api/v1/tender-bid-stances", headers=headers)
    assert listed.status_code == 200
    assert listed.json()[0]["id"] == body["id"]


def test_http_create_bad_stance_code_is_400(catalog_client: object) -> None:
    client, _stances, boards = catalog_client
    assert boards.row is not None
    response = client.post(
        "/api/v1/tender-bid-stances",
        headers=bearer_auth_headers(),
        json=_payload(boards.row.id, stance_code="won"),
    )
    assert response.status_code == 400
    assert "udział" in response.json()["detail"]


def test_http_create_stance_foreign_source_ref_is_400(catalog_client: object) -> None:
    client, _stances, boards = catalog_client
    assert boards.row is not None
    response = client.post(
        "/api/v1/tender-bid-stances",
