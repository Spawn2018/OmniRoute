...
_DENIED = "Brak uprawnienia can_manage_relation_document_requirements na organization"
_CALLS = (
    ("GET", "/api/v1/relation-document-requirements", None, None),
    (
        "POST",
        "/api/v1/relation-document-requirements",
        None,
        {
            "requirement_code": "postal_01",
...
class DenyShipmentMonitoringAuthz:
    async def check(
        self,
        *,
        user_id: UUID,
...
def test_relation_document_requirement_endpoints_are_forbidden_without_permission(
    method: str,
    path: str,
    params: dict[str, str] | None,
    json_body: dict[str, object] | None,
) -> None:
    set_authz_checker(DenyShipmentMonitoringAuthz())
