...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_seal_in_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="sl")
    loc_a = _zone(organization_id=org_a.id, created_by=user_a.id, code="PL-S", name="Strefa S")
    session.add(loc_a)
    await session.flush()
    session.add(
        Stop(
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].seal_in == "ABC123"


@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_seal_out_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="so")
    loc_a = _zone(organization_id=org_a.id, created_by=user_a.id, code="PL-O", name="Strefa O")
    session.add(loc_a)
    await session.flush()
    session.add(
        Stop(
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].seal_out == "XYZ"


@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_appointment_ref_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="ap")
    loc_a = _zone(organization_id=org_a.id, created_by=user_a.id, code="PL-A", name="Strefa A")
    session.add(loc_a)
    await session.flush()
    session.add(
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].appointment_ref == "WH-12"


@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_waiting_free_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="wt")
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].waiting_free_minutes == 15


@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_waiting_started_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="ws")
    loc_a = _zone(organization_id=org_a.id, created_by=user_a.id, code="PL-X", name="Strefa X")
    session.add(loc_a)
    await session.flush()
    session.add(
        Stop(
            ...
            time_zone="Europe/Warsaw",
            status="pending",
            source_ref="fixture://stop/x1",
            waiting_started_at=_CLOCK,
            eta_physical=_CLOCK,
            eta_legal=_CLOCK,
            created_by=user_a.id,
        ),
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].waiting_started_at == _CLOCK


@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_no_show_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="ns")
    loc_a = _zone(organization_id=org_a.id, created_by=user_a.id, code="PL-N", name="Strefa N")
    session.add(loc_a)
    await session.flush()
    session.add(
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].no_show_at == _CLOCK


@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_weigh_in_kg_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="wi")
    loc_a = _zone(organization_id=org_a.id, created_by=user_a.id, code="PL-W", name="Strefa W")
    session.add(loc_a)
    await session.flush()
    session.add(
        Stop(
            ...
            time_zone="Europe/Warsaw",
            status="pending",
            source_ref="fixture://stop/wi1",
            weigh_in_kg=Decimal("18.2500"),
            eta_physical=_CLOCK,
            eta_legal=_CLOCK,
            created_by=user_a.id,
        ),
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].weigh_in_kg == Decimal("18.2500")


@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_weigh_out_kg_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="wo")
    loc_a = _zone(organization_id=org_a.id, created_by=user_a.id, code="PL-X", name="Strefa X")
    session.add(loc_a)
    await session.flush()
    session.add(
        Stop(
            ...
            time_zone="Europe/Warsaw",
            status="pending",
            source_ref="fixture://stop/wo1",
            weigh_out_kg=Decimal("17.5000"),
            eta_physical=_CLOCK,
            eta_legal=_CLOCK,
            created_by=user_a.id,
        ),
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].weigh_out_kg == Decimal("17.5000")


@pytest.mark.integration
@pytest.mark.asyncio
async def test_stop_pod_quality_same_tenant(session, two_tenants) -> None:
    org_a = two_tenants["org_a"]
    user_a = two_tenants["user_a"]
    await bind_tenant(session, org_a.id)
    ship_a = await _booked(session, organization_id=org_a.id, user_id=user_a.id, suffix="pq")
    loc_a = _zone(organization_id=org_a.id, created_by=user_a.id, code="PL-Y", name="Strefa Y")
    session.add(loc_a)
    await session.flush()
    session.add(
        Stop(
    ...
    await bind_tenant(session, org_a.id)
    loaded = list((await session.scalars(select(Stop))).all())
    assert loaded[0].pod_quality == "ok"
