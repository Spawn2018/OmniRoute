...
def test_migration_281_creates_mqc_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/281_mqc_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "281_mqc_mark"' in source
    assert 'down_revision: str | None = "280_fair_share_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "mqc_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.mqc_marks" in forbidden
    assert "app.models.mqc_mark" in forbidden


def test_api_types_include_mqc_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "MqcMarkResponse" in source
    assert "MqcMarkCreate" in source


def test_fga_source_declares_mqc_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_mqc_marks: member" in source


def test_fga_model_grants_mqc_to_member() -> None:
    organization = next(
        definition
        for definition in authorization_model_request().type_definitions
        if definition.type == "organization"
    )
    relation = organization.relations["can_manage_mqc_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.mqc_marks.MqcMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "mqc_act_01",
        "mqc_kind": "mqc",
        "source_ref": "fixture://mqc-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    assert response.json()["mqc_kind"] == "mqc"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/mqc-marks",
        headers=bearer_auth_headers(),
        json=_payload(mqc_kind="live_mqc"),
    )
    assert response.status_code == 400
