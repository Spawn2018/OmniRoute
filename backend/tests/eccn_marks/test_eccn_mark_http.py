...
def test_migration_282_creates_eccn_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/282_eccn_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "282_eccn_mark"' in source
    assert 'down_revision: str | None = "281_mqc_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "eccn_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.eccn_marks" in forbidden
    assert "app.models.eccn_mark" in forbidden


def test_api_types_include_eccn_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "EccnMarkResponse" in source
    assert "EccnMarkCreate" in source


def test_fga_source_declares_eccn_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_eccn_marks: member" in source


def test_fga_model_grants_eccn_to_member() -> None:
    organization = next(
        definition
        for definition in authorization_model_request().type_definitions
        if definition.type == "organization"
    )
    relation = organization.relations["can_manage_eccn_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.eccn_marks.EccnMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "eccn_ear_01",
        "control_kind": "eccn",
        "source_ref": "fixture://eccn-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    assert response.json()["control_kind"] == "eccn"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/eccn-marks",
        headers=bearer_auth_headers(),
        json=_payload(control_kind="live_eccn"),
    )
    assert response.status_code == 400
