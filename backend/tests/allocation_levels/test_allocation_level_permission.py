...
_DENIED = "Brak uprawnienia can_manage_allocation_levels na organization"

_CALLS = (
    ("GET", "/api/v1/allocation-levels", None, None),
    (
        "POST",
        "/api/v1/allocation-levels",
        None,
        {"level_code": "tier_one", "source_ref": "fixture://allocation-level/a"},
    ),
)


class DenyAllocationLevelAuthz:
    async def check(
        self,
        *,
        user_id: UUID,
...
def test_allocation_level_endpoints_are_forbidden_without_permission(
    method: str,
    path: str,
    params: dict[str, str] | None,
    json_body: dict[str, object] | None,
) -> None:
    set_authz_checker(DenyAllocationLevelAuthz())
