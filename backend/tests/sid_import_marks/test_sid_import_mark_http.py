...
def test_migration_270_creates_sid_import_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/270_sid_import_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "270_sid_import_mark"' in source
    assert 'down_revision: str | None = "269_peppol_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "sid_import_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.sid_import_marks" in forbidden
    assert "app.models.sid_import_mark" in forbidden


def test_api_types_include_sid_import_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "SidImportMarkResponse" in source
    assert "SidImportMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_sid_import_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_sid_import_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.sid_import_marks.SidImportMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "sid_batch_01",
        "sid_kind": "sid",
        "source_ref": "fixture://sid-import-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/sid-import-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["sid_kind"] == "sid"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/sid-import-marks",
        headers=bearer_auth_headers(),
        json=_payload(sid_kind="live_sid"),
    )
    assert response.status_code == 400
