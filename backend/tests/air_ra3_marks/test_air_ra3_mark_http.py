...
def test_migration_291_creates_air_ra3_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/291_air_ra3_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "291_air_ra3_mark"' in source
    assert 'down_revision: str | None = "290_csrd_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "air_ra3_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.air_ra3_marks" in forbidden
    assert "app.models.air_ra3_mark" in forbidden


def test_api_types_include_air_ra3_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "AirRa3MarkResponse" in source
    assert "AirRa3MarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_air_ra3_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_air_ra3_marks"]
...
class InMemoryFleetCostDesk:
    ...
    async def persist_air_ra3_mark(
        ...
        *,
        ...
        code, kind, origin = parse_air_ra3_mark_row(
            mark_code,
            air_kind,
            source_ref,
        )
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.air_ra3_marks.AirRa3MarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "air3_ra_01",
        "air_kind": "ra3",
        "source_ref": "fixture://air-ra3-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/air-ra3-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["air_kind"] == "ra3"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/air-ra3-marks",
        headers=bearer_auth_headers(),
        json=_payload(air_kind="live_ra3"),
    )
    assert response.status_code == 400
