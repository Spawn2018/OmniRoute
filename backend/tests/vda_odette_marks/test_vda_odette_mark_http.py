...
def test_migration_278_creates_vda_odette_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/278_vda_odette_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "278_vda_odette_mark"' in source
    assert 'down_revision: str | None = "277_jit_jis_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "vda_odette_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.vda_odette_marks" in forbidden
    assert "app.models.vda_odette_mark" in forbidden


def test_api_types_include_vda_odette_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "VdaOdetteMarkResponse" in source
    assert "VdaOdetteMarkCreate" in source


def test_fga_source_declares_vda_odette_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_vda_odette_marks: member" in source


def test_fga_model_grants_vda_odette_to_member() -> None:
    organization = next(
        definition
        for definition in authorization_model_request().type_definitions
        if definition.type == "organization"
    )
    relation = organization.relations["can_manage_vda_odette_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.vda_odette_marks.VdaOdetteMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "vo_vda_01",
        "edi_kind": "vda",
        "source_ref": "fixture://vda-odette-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/vda-odette-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["edi_kind"] == "vda"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/vda-odette-marks",
        headers=bearer_auth_headers(),
        json=_payload(edi_kind="live_vda"),
    )
    assert response.status_code == 400
