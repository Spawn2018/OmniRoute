...
def test_http_create_list_supersede_and_reject_check_digit(box_client: object) -> None:
    ...
    assert first.json()["alliance_service"] is None
...
def test_http_create_container_with_alliance_service(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    created = client.post(
        "/api/v1/containers",
    ...
    assert created.status_code == 201
    assert created.json()["alliance_service"] == "2M"
    assert created.json()["vessel_imo"] is None


def test_http_rejects_non_text_alliance_service(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    reply = client.post(
        "/api/v1/containers",
...
def test_http_rejects_too_long_alliance_service(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    reply = client.post(
        "/api/v1/containers",
...
def test_http_create_container_with_return_date(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    created = client.post(
        "/api/v1/containers",
...
def test_http_rejects_timestamp_container_return(box_client: object) -> None:
    ...
    reply = client.post(
        ...
        json={
            ...
            "iso_size_type": "22G1",
            "source_ref": "tenant:manual",
            "return_date": "2026-09-20T00:00:00",
        },
    )
    assert reply.status_code == 400
...
def test_http_create_container_with_gate_in_date(box_client: object) -> None:
    ...
    assert created.json()["return_date"] is None
    assert created.json()["delivery_date"] is None
...
def test_http_create_container_with_delivery_date(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    created = client.post(
        "/api/v1/containers",
    ...
    assert created.status_code == 201
    assert created.json()["delivery_date"] == "2026-09-22"
    assert created.json()["gate_in_date"] is None
    assert created.json()["unload_date"] is None


def test_http_rejects_timestamp_container_delivery(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    reply = client.post(
        "/api/v1/containers",
        ...
        json={
            ...
            "iso_size_type": "22G1",
            "source_ref": "tenant:manual",
            "delivery_date": "2026-09-22T00:00:00",
        },
    )
    assert reply.status_code == 400
    assert "data dostawy" in reply.json()["detail"]


def test_http_create_container_with_unload_date(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    created = client.post(
        "/api/v1/containers",
        ...
        json={
            ...
            "iso_size_type": "22G1",
            "source_ref": "tenant:manual",
            "unload_date": "2026-09-23",
        },
    )
    assert created.status_code == 201
    assert created.json()["unload_date"] == "2026-09-23"
    assert created.json()["delivery_date"] is None
    assert created.json()["temp_min"] is None


def test_http_rejects_timestamp_container_unload(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    reply = client.post(
        "/api/v1/containers",
        ...
        json={
            ...
            "iso_size_type": "22G1",
            "source_ref": "tenant:manual",
            "unload_date": "2026-09-23T00:00:00",
        },
    )
    assert reply.status_code == 400
    assert "data rozładunku" in reply.json()["detail"]


def test_http_create_container_with_temp_min(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    created = client.post(
        "/api/v1/containers",
...
def test_http_rejects_float_container_temp_min(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    reply = client.post(
        "/api/v1/containers",
...
def test_http_create_container_with_temp_max(box_client: object) -> None:
    client, _boxes, _jobs = box_client
    headers = bearer_auth_headers()
    created = client.post(
        "/api/v1/containers",
