...
def test_migration_486_adds_pod_unlocode_without_port_fk() -> None:
    source = (
        _ROOT / "backend" / "alembic" / "versions" / "486_container_pod_unlocode.py"
    ).read_text(encoding="utf-8")
    assert 'revision: str = "486_container_pod_unlocode"' in source
    assert 'down_revision: str | None = "485_container_pol_unlocode"' in source
...
def test_migration_462_adds_temp_max_without_float() -> None:
    source = (
        _ROOT / "backend" / "alembic" / "versions" / "462_container_temp_max.py"
    ).read_text(encoding="utf-8")
    assert 'revision: str = "462_container_temp_max"' in source
    assert 'down_revision: str | None = "461_container_temp_min"' in source
    assert "temp_max" in source
    assert "Numeric(14, 4)" in source
    assert "float" not in source.split("def upgrade")[1]
    assert "temp_min" not in source.split("def upgrade")[1]
    assert "httpx" not in source
    assert "create_table" not in source
    assert "def downgrade" in source
    assert "temp_max" in source.split("def downgrade")[1]


def test_migration_463_adds_needs_external_power_without_live() -> None:
    source = (
        _ROOT / "backend" / "alembic" / "versions" / "463_container_ext_power.py"
    ).read_text(encoding="utf-8")
    assert 'revision: str = "463_container_ext_power"' in source
    assert 'down_revision: str | None = "462_container_temp_max"' in source
    assert "needs_external_power" in source
