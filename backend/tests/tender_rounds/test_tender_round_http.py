...
def catalog_client(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr("app.api.tender_rounds.TenderRoundService", lambda _s: turns)
    monkeypatch.setattr("app.api.tender_rounds.TenderService", lambda _s: boards)
    set_authz_checker(AllowAllAuthz())
    app.dependency_overrides[require_tenant_session] = _fake_tenant_session
    yield TestClient(app), turns, boards
...
def _payload(tender_id: UUID, **overrides: object) -> dict[str, object]:
    body: dict[str, object] = {
        ...
        "round_no": 1,
        "source_ref": "fixture://tender-round/1",
    }
    body.update(overrides)
    return body


def test_http_create_and_list_tender_round(catalog_client: object) -> None:
    client, _turns, boards = catalog_client
    assert boards.row is not None
    org_id = uuid4()
    headers = bearer_auth_headers(organization_id=org_id)
    created = client.post(
        "/api/v1/tender-rounds",
        headers=headers,
    ...
    listed = client.get("/api/v1/tender-rounds", headers=headers)
    assert listed.status_code == 200
    assert listed.json()[0]["id"] == body["id"]


def test_http_create_round_zero_is_400(catalog_client: object) -> None:
    client, _turns, boards = catalog_client
    assert boards.row is not None
    response = client.post(
        "/api/v1/tender-rounds",
        headers=bearer_auth_headers(),
        json=_payload(boards.row.id, round_no=0),
    )
    assert response.status_code == 400
    assert "runda" in response.json()["detail"]


def test_http_create_round_foreign_source_ref_is_400(catalog_client: object) -> None:
    client, _turns, boards = catalog_client
    assert boards.row is not None
    response = client.post(
        "/api/v1/tender-rounds",
