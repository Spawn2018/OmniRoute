...
def test_migration_297_creates_multi_manning_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/297_multi_manning_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "297_multi_manning_mark"' in source
    assert 'down_revision: str | None = "296_nvocc_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "multi_manning_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.multi_manning_marks" in forbidden
    assert "app.models.multi_manning_mark" in forbidden


def test_api_types_include_multi_manning_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "MultiManningMarkResponse" in source
    assert "MultiManningMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_multi_manning_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_multi_manning_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[MultiManningMark] = []
    ...
    async def persist_multi_manning_mark(
        ...
        *,
        ...
        code, kind, origin = parse_multi_manning_mark_row(
            mark_code,
            manning_kind,
            source_ref,
        )
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.multi_manning_marks.MultiManningMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "mm_dual_01",
        "manning_kind": "dual",
        "source_ref": "fixture://multi-manning-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/multi-manning-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["manning_kind"] == "dual"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/multi-manning-marks",
        headers=bearer_auth_headers(),
        json=_payload(manning_kind="live_manning"),
    )
    assert response.status_code == 400
