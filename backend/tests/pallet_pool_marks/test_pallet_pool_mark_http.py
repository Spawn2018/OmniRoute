...
def test_migration_266_creates_pallet_pool_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/266_pallet_pool_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "266_pallet_pool_mark"' in source
    assert 'down_revision: str | None = "265_bin_pack_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "pallet_pool_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.pallet_pool_marks" in forbidden
    assert "app.models.pallet_pool_mark" in forbidden


def test_api_types_include_pallet_pool_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "PalletPoolMarkResponse" in source
    assert "PalletPoolMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_pallet_pool_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_pallet_pool_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.pallet_pool_marks.PalletPoolMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "pool_chep_01",
        "pool_kind": "chep",
        "source_ref": "fixture://pallet-pool-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/pallet-pool-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["pool_kind"] == "chep"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/pallet-pool-marks",
        headers=bearer_auth_headers(),
        json=_payload(pool_kind="live_chep"),
    )
    assert response.status_code == 400
