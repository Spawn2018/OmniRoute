...
class StubResourceService:
    ...
    async def record_resource(
        ...
        *,
        ...
        profile = require_vehicle_profile(vehicle_profile)
        ...
        if (
            ...
            and current.vehicle_profile == profile
...
def test_http_create_list_supersede_and_reject_truck(fleet_client: object) -> None:
    ...
    assert first.json()["vehicle_profile"] is None
...
def test_http_tail_lift_supersedes(fleet_client: object) -> None:
    client, _rows = fleet_client
    headers = bearer_auth_headers(organization_id=uuid4())
    payload = {
        "resource_kind": "vehicle",
    ...
    first = client.post("/api/v1/resources", headers=headers, json=payload)
    assert first.status_code == 201
    assert first.json()["tail_lift"] is True
    same = client.post("/api/v1/resources", headers=headers, json=payload)
    assert same.json()["id"] == first.json()["id"]
    changed = client.post(
        "/api/v1/resources",
        headers=headers,
        json={**payload, "tail_lift": False},
    )
    assert changed.status_code == 201
    assert changed.json()["id"] != first.json()["id"]
    bad = client.post(
        "/api/v1/resources",
        headers=headers,
        json={**payload, "display_name": "Inna", "tail_lift": "tak"},
    )
    assert bad.status_code == 400
    assert "winda" in bad.json()["detail"]


def test_http_phone_supersedes(fleet_client: object) -> None:
    client, _rows = fleet_client
    headers = bearer_auth_headers(organization_id=uuid4())
    payload = {
    ...
    first = client.post("/api/v1/resources", headers=headers, json=payload)
    assert first.status_code == 201
    assert first.json()["phone"] == "+48 500"
    same = client.post("/api/v1/resources", headers=headers, json=payload)
    assert same.json()["id"] == first.json()["id"]
    changed = client.post(
        "/api/v1/resources",
        headers=headers,
        json={**payload, "phone": "+48 600"},
    )
    assert changed.status_code == 201
    assert changed.json()["id"] != first.json()["id"]
    too_long = client.post(
        "/api/v1/resources",
        headers=headers,
        json={**payload, "display_name": "Inna", "phone": "1" * 65},
    )
    assert too_long.status_code == 400
...
def test_http_vehicle_profile_supersedes(fleet_client: object) -> None:
    client, _rows = fleet_client
    headers = bearer_auth_headers(organization_id=uuid4())
    payload = {
        "resource_kind": "vehicle",
        "display_name": "MAN",
        "source_ref": "tenant:manual",
        "vehicle_profile": " HERE ",
    }
    first = client.post("/api/v1/resources", headers=headers, json=payload)
    assert first.status_code == 201
    assert first.json()["vehicle_profile"] == "HERE"
    same = client.post("/api/v1/resources", headers=headers, json=payload)
    assert same.json()["id"] == first.json()["id"]
    changed = client.post(
        "/api/v1/resources",
        headers=headers,
        json={**payload, "vehicle_profile": "TOMTOM"},
    )
    assert changed.status_code == 201
    assert changed.json()["id"] != first.json()["id"]
    too_long = client.post(
        "/api/v1/resources",
        headers=headers,
        json={**payload, "display_name": "Inna", "vehicle_profile": "1" * 65},
    )
    assert too_long.status_code == 400
    assert "profil" in too_long.json()["detail"]
