...
class StubTedDesk:
    def __init__(self, session: object) -> None:
        self._session = session
        self.rows: list[TenderTedNotice] = []
...
def catalog_client(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.tender_ted_notices.TenderTedNoticeService",
        lambda _s: notices,
    )
    monkeypatch.setattr("app.api.tender_ted_notices.TenderService", lambda _s: boards)
    set_authz_checker(AllowAllAuthz())
    app.dependency_overrides[require_tenant_session] = _fake_tenant_session
    yield TestClient(app), notices, boards
...
def _payload(tender_id: UUID, **overrides: object) -> dict[str, object]:
    body: dict[str, object] = {
        ...
        "source_ref": "fixture://tender-ted-notice/1",
    }
    body.update(overrides)
    return body


def test_http_create_and_list_tender_ted_notice(catalog_client: object) -> None:
    client, _notices, boards = catalog_client
    assert boards.row is not None
    org_id = uuid4()
    headers = bearer_auth_headers(organization_id=org_id)
    created = client.post(
        "/api/v1/tender-ted-notices",
        headers=headers,
    ...
    assert body["notice_number"] == "123456-2024"
    assert "buy_amount" not in body
    listed = client.get("/api/v1/tender-ted-notices", headers=headers)
    assert listed.status_code == 200
    assert listed.json()[0]["id"] == body["id"]


def test_http_create_bad_notice_number_is_400(catalog_client: object) -> None:
    client, _notices, boards = catalog_client
    assert boards.row is not None
    response = client.post(
        "/api/v1/tender-ted-notices",
        headers=bearer_auth_headers(),
        json=_payload(boards.row.id, notice_number="http://ted.europa.eu/x"),
    )
    assert response.status_code == 400
    assert "ogłoszenie" in response.json()["detail"]


def test_http_create_ted_foreign_source_ref_is_400(catalog_client: object) -> None:
    client, _notices, boards = catalog_client
    assert boards.row is not None
    response = client.post(
        "/api/v1/tender-ted-notices",
