...
_DENIED = "Brak uprawnienia can_manage_autonomy_levels na organization"

_CALLS = (
    ("GET", "/api/v1/autonomy-levels", None, None),
    (
        "POST",
        "/api/v1/autonomy-levels",
        None,
        {"level_code": "observer", "source_ref": "fixture://autonomy-level/a"},
    ),
)


class DenyAutonomyLevelAuthz:
    async def check(
        self,
        *,
        user_id: UUID,
...
def test_autonomy_level_endpoints_are_forbidden_without_permission(
    method: str,
    path: str,
    params: dict[str, str] | None,
    json_body: dict[str, object] | None,
) -> None:
    set_authz_checker(DenyAutonomyLevelAuthz())
