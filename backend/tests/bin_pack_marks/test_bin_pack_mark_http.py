...
def test_migration_265_creates_bin_pack_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/265_bin_pack_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "265_bin_pack_mark"' in source
    assert 'down_revision: str | None = "264_fleet_cost_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "bin_pack_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.bin_pack_marks" in forbidden
    assert "app.models.bin_pack_mark" in forbidden


def test_api_types_include_bin_pack_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "BinPackMarkResponse" in source
    assert "BinPackMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_bin_pack_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_bin_pack_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.bin_pack_marks.BinPackMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "pack_vol_01",
        "pack_kind": "volume",
        "source_ref": "fixture://bin-pack-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/bin-pack-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["pack_kind"] == "volume"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/bin-pack-marks",
        headers=bearer_auth_headers(),
        json=_payload(pack_kind="live_or"),
    )
    assert response.status_code == 400
