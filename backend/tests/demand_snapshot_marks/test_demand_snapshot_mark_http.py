...
def test_migration_289_creates_demand_snapshot_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/289_demand_snapshot_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "289_demand_snapshot_mark"' in source
    assert 'down_revision: str | None = "288_tender_decline_reason"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "demand_snapshot_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.demand_snapshot_marks" in forbidden
    assert "app.models.demand_snapshot_mark" in forbidden


def test_api_types_include_demand_snapshot_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "DemandSnapshotMarkResponse" in source
    assert "DemandSnapshotMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_demand_snapshot_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_demand_snapshot_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[DemandSnapshotMark] = []
    ...
    async def persist_demand_snapshot_mark(
        ...
        *,
        ...
        code, kind, origin = parse_demand_snapshot_mark_row(
            mark_code,
            snapshot_kind,
            source_ref,
        )
        row = DemandSnapshotMark(
            id=uuid4(),
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.demand_snapshot_marks.DemandSnapshotMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        ...
        "source_ref": "fixture://demand-snapshot-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/demand-snapshot-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["snapshot_kind"] == "forecast"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/demand-snapshot-marks",
        headers=bearer_auth_headers(),
        json=_payload(snapshot_kind="live_demand"),
    )
    assert response.status_code == 400
