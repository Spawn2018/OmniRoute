...
def test_migration_300_creates_lez_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/300_lez_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "300_lez_mark"' in source
    assert 'down_revision: str | None = "299_tacho_office_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "lez_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.lez_marks" in forbidden
    assert "app.models.lez_mark" in forbidden


def test_api_types_include_lez_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "LezMarkResponse" in source
    assert "LezMarkCreate" in source


def test_fga_source_declares_switch_bl_loi_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_lez_marks: member" in source
...
def test_fga_model_grants_switch_bl_loi_to_member() -> None:
    ...
    relation = organization.relations["can_manage_lez_marks"]
...
class InMemoryFleetCostDesk:
    ...
    async def persist_lez_mark(
        ...
        *,
        ...
        code, kind, origin = parse_lez_mark_row(
            mark_code,
            lez_kind,
            source_ref,
        )
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.lez_marks.LezMarkService",
        lambda _s: desk,
    )
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    assert response.json()["lez_kind"] == "lez"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/lez-marks",
        headers=bearer_auth_headers(),
        json=_payload(lez_kind="live_lez"),
    )
    assert response.status_code == 400
