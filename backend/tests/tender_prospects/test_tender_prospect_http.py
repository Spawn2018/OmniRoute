...
class StubProspectDesk:
    def __init__(self, session: object) -> None:
        self._session = session
        self.rows: list[TenderProspect] = []
...
def catalog_client(monkeypatch: pytest.MonkeyPatch) -> object:
    prospects = StubProspectDesk(object())
    ...
    monkeypatch.setattr(
        "app.api.tender_prospects.TenderProspectService",
        lambda _s: prospects,
    )
    monkeypatch.setattr("app.api.tender_prospects.TenderService", lambda _s: boards)
    monkeypatch.setattr("app.api.tender_prospects.PartyService", lambda _s: parties)
    set_authz_checker(AllowAllAuthz())
    app.dependency_overrides[require_tenant_session] = _fake_tenant_session
    yield TestClient(app), prospects, boards, parties
...
def _payload(tender_id: UUID, party_id: UUID, **overrides: object) -> dict[str, object]:
    body: dict[str, object] = {
        ...
        "outreach_code": "called",
        "source_ref": "fixture://tender-prospect/1",
    }
    body.update(overrides)
    return body


def test_http_create_and_list_tender_prospect(catalog_client: object) -> None:
    client, _prospects, boards, parties = catalog_client
    assert boards.row is not None
    assert parties.row is not None
    org_id = uuid4()
    headers = bearer_auth_headers(organization_id=org_id)
    created = client.post(
        "/api/v1/tender-prospects",
        headers=headers,
    ...
    assert body["outreach_code"] == "called"
    assert "buy_amount" not in body
    listed = client.get("/api/v1/tender-prospects", headers=headers)
    assert listed.status_code == 200
    assert listed.json()[0]["id"] == body["id"]


def test_http_create_bad_outreach_code_is_400(catalog_client: object) -> None:
    client, _prospects, boards, parties = catalog_client
    assert boards.row is not None
    assert parties.row is not None
    response = client.post(
        ...
        json=_payload(boards.row.id, parties.row.id, outreach_code="X"),
    )
    assert response.status_code == 400
    assert "prospekt" in response.json()["detail"]


def test_http_create_prospect_foreign_source_ref_is_400(catalog_client: object) -> None:
    client, _prospects, boards, parties = catalog_client
    assert boards.row is not None
    assert parties.row is not None
    response = client.post(
