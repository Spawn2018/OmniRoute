...
def test_migration_268_creates_e_delivery_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/268_e_delivery_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "268_e_delivery_mark"' in source
    assert 'down_revision: str | None = "267_e_cmr_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "e_delivery_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.e_delivery_marks" in forbidden
    assert "app.models.e_delivery_mark" in forbidden


def test_api_types_include_e_delivery_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "EDeliveryMarkResponse" in source
    assert "EDeliveryMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_e_delivery_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_e_delivery_marks"]
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.e_delivery_marks.EDeliveryMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "edor_01",
        "delivery_kind": "edor",
        "source_ref": "fixture://e-delivery-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/e-delivery-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["delivery_kind"] == "edor"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/e-delivery-marks",
        headers=bearer_auth_headers(),
        json=_payload(delivery_kind="live_edor"),
    )
    assert response.status_code == 400
