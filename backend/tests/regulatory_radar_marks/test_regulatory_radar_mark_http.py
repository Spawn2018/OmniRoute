...
def test_migration_274_creates_regulatory_radar_and_rls() -> None:
    source = (_ROOT / "backend/alembic/versions/274_regulatory_radar_mark.py").read_text(
        encoding="utf-8",
    )
    assert 'revision: str = "274_regulatory_radar_mark"' in source
    assert 'down_revision: str | None = "273_partner_exchange_mark"' in source
    assert "FORCE ROW LEVEL SECURITY" in source
    assert "regulatory_radar_mark_tenant_isolation" in source
...
def test_importlinter_lists_fleet_cost_on_deny() -> None:
    ...
    assert "app.services.regulatory_radar_marks" in forbidden
    assert "app.models.regulatory_radar_mark" in forbidden


def test_api_types_include_regulatory_radar_mark() -> None:
    source = (_ROOT / "frontend/src/api/types.gen.ts").read_text(encoding="utf-8")
    assert "RegulatoryRadarMarkResponse" in source
    assert "RegulatoryRadarMarkCreate" in source


def test_fga_source_declares_fleet_cost_relation() -> None:
    source = (_ROOT / "authz/model.fga").read_text(encoding="utf-8")
    assert "can_manage_regulatory_radar_marks: member" in source
...
def test_fga_model_grants_fleet_cost_to_member() -> None:
    ...
    relation = organization.relations["can_manage_regulatory_radar_marks"]
...
class InMemoryFleetCostDesk:
    def __init__(self, session: object) -> None:
        self.rows: list[RegulatoryRadarMark] = []
...
def fleet_cost_http(monkeypatch: pytest.MonkeyPatch) -> object:
    ...
    monkeypatch.setattr(
        "app.api.regulatory_radar_marks.RegulatoryRadarMarkService",
        lambda _s: desk,
    )
...
def _payload(**extra: object) -> dict[str, object]:
    body: dict[str, object] = {
        "mark_code": "rr_notice_01",
        "radar_kind": "notice",
        "source_ref": "fixture://regulatory-radar-mark/a",
    }
    body.update(extra)
    return body
...
def test_post_persists(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/regulatory-radar-marks",
        headers=bearer_auth_headers(),
        json=_payload(),
    )
    assert response.status_code == 201
    assert response.json()["radar_kind"] == "notice"
...
def test_post_rejects_bad_kind(fleet_cost_http: object) -> None:
    ...
    response = client.post(
        "/api/v1/regulatory-radar-marks",
        headers=bearer_auth_headers(),
        json=_payload(radar_kind="live_radar"),
    )
    assert response.status_code == 400
