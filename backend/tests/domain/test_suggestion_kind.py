...
def _ok(**extra: object) -> object:
    ...
    body.update(extra)
    return parse_suggestion_kind_row(body["kind_code"], body["source_ref"])


def test_parse_accepts_open_kind_outside_ledger_set() -> None:
    draft = _ok()
    assert draft.kind_code == "tender_twin"
...
def test_parse_rejects_bad_code() -> None:
    with pytest.raises(InvalidSuggestionKind, match="kod"):
        _ok(kind_code="X")


def test_parse_rejects_foreign_source() -> None:
    with pytest.raises(InvalidSuggestionKind, match="wskazanie"):
