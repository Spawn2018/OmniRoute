...
8e. `python scripts/quality/factory_cycle.py --close` — bench, karta z powtórzonych czerwonych CI, podłoga jakości tylko w górę. `just meta-gate` łapie OS-3: test z nowym API/serwisem, how-to albo leftover, delta zanim produkt. Operator nie redaguje. **Nie** dopisuj zasad do AGENTS. **Nie** ruszaj GROUNDING.
